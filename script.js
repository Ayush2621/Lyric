/* ========== Sample songs ========== 
       Using public sample MP3s (SoundHelix). Replace with your own files/URLs when packaging as an app.
    */
   /* --- AUTOMATICALLY GENERATED SONGS DB (2025-11-09) --- */

 /* --- AUTOMATICALLY GENERATED SONGS DB (2025-11-09) --- */

/* --- AUTOMATICALLY GENERATED SONGS DB (2025-11-13) --- */

/* --- AUTOMATICALLY GENERATED SONGS DB (2025-11-13) --- */

/* --- AUTOMATICALLY GENERATED SONGS DB (2025-11-13) --- */

const songsDB = [
    {
        "name": "; Desi Kalakaar Song ¦ Yo Yo Honey Singh ¦ Honey Singh New Songs 2014",
        "artist": "Unknown Artist",
        "src": "music/Official%3B%20Desi%20Kalakaar%20Full%20VIDEO%20Song%20%C2%A6%20Yo%20Yo%20Honey%20Singh%20%C2%A6%20Honey%20Singh%20New%20Songs%202014%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "246"
    },
    {
        "name": "; Kabhi Kabhi Aditi Zindagi ¦ Jaane Tu Ya Jaane Na ¦ A R Rahman ¦ Rashid Ali",
        "artist": "Unknown Artist",
        "src": "music/Full%20Video%3B%20Kabhi%20Kabhi%20Aditi%20Zindagi%20%C2%A6%20Jaane%20Tu%20Ya%20Jaane%20Na%20%C2%A6%20A.R.%20Rahman%20%C2%A6%20Rashid%20Ali%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "136"
    },
    {
        "name": "; KISSIK Song ¦ Pushpa 2 The Rule ¦ Allu Arjun ¦ Sukumar ¦ Sreeleela ¦ DSP",
        "artist": "Unknown Artist",
        "src": "music/Full%20Video%3B%20KISSIK%20Song%20%C2%A6%20Pushpa%202%20The%20Rule%20%C2%A6%20Allu%20Arjun%20%C2%A6%20Sukumar%20%C2%A6%20Sreeleela%20%C2%A6%20DSP%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "137"
    },
    {
        "name": "; Kole Bheja ; Luv Sharma ¦ Seera Buttar ¦ Harssh Dada",
        "artist": "Unknown Artist",
        "src": "music/Official%20Audio%20%3B%20%20Kole%20Bheja%20%3B%20Luv%20Sharma%20%C2%A6%20Seera%20Buttar%20%C2%A6%20Harssh%20Dada%20(160kbit_Opus).mp3",
        "color": "#ef4444",
        "id": "245"
    },
    {
        "name": "; Masakali ¦ Delhi 6 ¦ Abhishek Bachchan, Sonam Kapoor ¦ A R Rahman ¦ Mohit Chauhan",
        "artist": "Unknown Artist",
        "src": "music/Full%20Video%3B%20Masakali%20%C2%A6%20Delhi%206%20%C2%A6%20Abhishek%20Bachchan%2C%20Sonam%20Kapoor%20%C2%A6%20A.R.%20Rahman%20%C2%A6%20%20Mohit%20Chauhan%20(152kbit_Opus).mp3",
        "color": "#f97316",
        "id": "138"
    },
    {
        "name": "'Chittiyaan Kalaiyaan' SONG ¦ Roy ¦ Meet Bros Anjjan, Kanika Kapoor ¦ T-SERIES",
        "artist": "Unknown Artist",
        "src": "music/'Chittiyaan%20Kalaiyaan'%20FULL%20VIDEO%20SONG%20%C2%A6%20Roy%20%C2%A6%20Meet%20Bros%20Anjjan%2C%20Kanika%20Kapoor%20%C2%A6%20T-SERIES%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "3"
    },
    {
        "name": "'Gallan Goodiyaan' Song ¦ Dil Dhadakne Do ¦ T-Series",
        "artist": "Unknown Artist",
        "src": "music/'Gallan%20Goodiyaan'%20Full%20VIDEO%20Song%20%C2%A6%20Dil%20Dhadakne%20Do%20%C2%A6%20T-Series%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "4"
    },
    {
        "name": "'Kabira ' Yeh Jawaani Hai Deewani ¦ Pritam ¦ Ranbir Kapoor, Deepika Padukone",
        "artist": "Unknown Artist",
        "src": "music/'Kabira%20Full%20Song'%20Yeh%20Jawaani%20Hai%20Deewani%20%C2%A6%20Pritam%20%C2%A6%20Ranbir%20Kapoor%2C%20Deepika%20Padukone%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "5"
    },
    {
        "name": "'حصريا نجوى فاروق ; 'خلوني نعيش",
        "artist": "Exclusive ; Najwa Farouk 'Khalouni N3ich'",
        "src": "music/Exclusive%20%3B%20Najwa%20Farouk%20'Khalouni%20N3ich'%20-%20'%D8%AD%D8%B5%D8%B1%D9%8A%D8%A7%20%D9%86%D8%AC%D9%88%D9%89%20%D9%81%D8%A7%D8%B1%D9%88%D9%82%20%3B%20'%D8%AE%D9%84%D9%88%D9%86%D9%8A%20%D9%86%D8%B9%D9%8A%D8%B4%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "129"
    },
    {
        "name": "@GeorgianFolk Acharuli Gandagana",
        "artist": "Unknown Artist",
        "src": "music/%40GeorgianFolk%20Acharuli%20Gandagana%20(Trap%20Remix%20Edit)%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "10"
    },
    {
        "name": "#MAFIA# ¦ Aggressive Mafia Trap Rap Beat Instrumental ¦ Mafya Müziği ¦ Prod by Pasha Music",
        "artist": "Unknown Artist",
        "src": "music/%23MAFIA%23%20%C2%A6%20Aggressive%20Mafia%20Trap%20Rap%20Beat%20Instrumental%20%C2%A6%20Mafya%20M%C3%BCzi%C4%9Fi%20%C2%A6%20Prod%20by%20Pasha%20Music%20(152kbit_Opus).mp3",
        "color": "#ef4444",
        "id": "1"
    },
    {
        "name": "¦ Gehraiyaan ¦ Deepika Padukone, Siddhant, Ananya, Dhairya ¦ OAFF, Savera",
        "artist": "Doobey",
        "src": "music/Doobey%20-%20Official%20Video%20%C2%A6%20Gehraiyaan%20%C2%A6%20Deepika%20Padukone%2C%20Siddhant%2C%20Ananya%2C%20Dhairya%20%C2%A6%20OAFF%2C%20Savera%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "108"
    },
    {
        "name": "¦ Honey 3 0 ¦ Yo Yo Honey Singh & Sonakshi Sinha ¦ Zee Music Originals",
        "artist": "KALAASTAR",
        "src": "music/KALAASTAR%20-%20Full%20Video%20%C2%A6%20Honey%203.0%20%C2%A6%20Yo%20Yo%20Honey%20Singh%20%26%20Sonakshi%20Sinha%20%C2%A6%20Zee%20Music%20Originals%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "186"
    },
    {
        "name": "¦ Jaundya Na Balasaheb ¦ Ajay-Atul ¦ Girish Kulkarni & Saie Tamhankar",
        "artist": "Dolby Walya",
        "src": "music/Dolby%20Walya%20-%20Full%20Video%20%C2%A6%20Jaundya%20Na%20Balasaheb%20%C2%A6%20Ajay-Atul%20%C2%A6%20Girish%20Kulkarni%20%26%20Saie%20Tamhankar%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "107"
    },
    {
        "name": "¦ Shershaah ¦ Sidharth - Kiara ¦ Tanishk B¦ Jubin Nautiyal ¦Asees",
        "artist": "Raataan Lambiyan",
        "src": "music/Raataan%20Lambiyan%20%E2%80%93%20Official%20Video%20%C2%A6%20Shershaah%20%C2%A6%20Sidharth%20%E2%80%93%20Kiara%20%C2%A6%20Tanishk%20B%C2%A6%20Jubin%20Nautiyal%20%20%C2%A6Asees%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "270"
    },
    {
        "name": "¦ Table No 21 ¦ Rajeev Khandelwal & Tina Desai ¦ Gajendra Verma",
        "artist": "Mann Mera",
        "src": "music/Mann%20Mera%20-%20Official%20Video%20%C2%A6%20Table%20No%2021%20%C2%A6%20Rajeev%20Khandelwal%20%26%20Tina%20Desai%20%C2%A6%20Gajendra%20Verma%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "219"
    },
    {
        "name": "¦¦Na cheli rojave song ¦¦A R Rahman",
        "artist": "Unknown Artist",
        "src": "music/%C2%A6%C2%A6Na%20cheli%20rojave%20song%20lyrics%C2%A6%C2%A6A.R.Rahman%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "355"
    },
    {
        "name": "6YNTHMANE x RXDXVIL 'BRAZILIAN DANÇA PHONK'",
        "artist": "Unknown Artist",
        "src": "music/6YNTHMANE%20x%20RXDXVIL%20'BRAZILIAN%20DAN%C3%87A%20PHONK'%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "8"
    },
    {
        "name": "9;45 feat Jay Trak",
        "artist": "Prabh",
        "src": "music/Prabh%20-%209%3B45%20(Official%20Visualizer)%20feat.%20Jay%20Trak%20(152kbit_Opus).mp3",
        "color": "#22c55e",
        "id": "260"
    },
    {
        "name": "A R Rahman ¦ Guru ¦ Aishwarya Rai ¦ Abhishek Bachchan",
        "artist": "Tere Bina",
        "src": "music/Tere%20Bina%20Full%20Video%20-%20A.%20R.%20Rahman%20%C2%A6%20Guru%20%C2%A6%20Aishwarya%20Rai%20%C2%A6%20Abhishek%20Bachchan%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "314"
    },
    {
        "name": "Aagadu Song ¦ Junction Lo Song ¦ Mahesh Babu, Shruti Haasan, Tamannaah Bhatia ¦Thaman S",
        "artist": "Unknown Artist",
        "src": "music/Aagadu%20Video%20Song%20%C2%A6%20Junction%20Lo%20Video%20Song%20%C2%A6%20Mahesh%20Babu%2C%20Shruti%20Haasan%2C%20Tamannaah%20Bhatia%20%C2%A6Thaman%20S%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "11"
    },
    {
        "name": "Aaj Ki Raat ¦ Stree 2 ¦ Tamannaah Bhatia ¦ Sachin–Jigar ¦ Madhubanti Bagchi ¦ Divya K ¦ Am…",
        "artist": "Unknown Artist",
        "src": "music/Aaj%20Ki%20Raat%20%C2%A6%20Stree%202%20%C2%A6%20Tamannaah%20Bhatia%20%C2%A6%20Sachin%E2%80%93Jigar%20%C2%A6%20Madhubanti%20Bagchi%20%C2%A6%20Divya%20K%20%C2%A6%20Amitabh%20B%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "12"
    },
    {
        "name": "Aasman Ko Chukar Dekha I Return Of Hanuman",
        "artist": "Unknown Artist",
        "src": "music/Aasman%20Ko%20Chukar%20Dekha%20I%20Return%20Of%20Hanuman%20(Animation)%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "15"
    },
    {
        "name": "Abhi Mujh Mein Kahin Best ¦Agneepath¦Priyanka Chopra,Hrithik¦Sonu Nigam",
        "artist": "Ajay-Atul",
        "src": "music/Ajay-Atul%20-%20Abhi%20Mujh%20Mein%20Kahin%20Best%20Lyric%C2%A6Agneepath%C2%A6Priyanka%20Chopra%2CHrithik%C2%A6Sonu%20Nigam%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "18"
    },
    {
        "name": "Addy Nagar ¦ Music ¦ @AddyNagar",
        "artist": "Kaley Sheshe",
        "src": "music/Kaley%20Sheshe%20%E2%80%93%20Addy%20Nagar%20%C2%A6%20Official%20Music%20Video%20%C2%A6%20%40AddyNagar%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "189"
    },
    {
        "name": "Aditya A",
        "artist": "Chaand Baaliyan",
        "src": "music/Chaand%20Baaliyan%20-%20Aditya%20A.%20(Official%20Video)%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "65"
    },
    {
        "name": "Aditya A ¦ Naalayak ¦ Ronit Vinta",
        "artist": "Sunday",
        "src": "music/Sunday%20(audio)%20-%20Aditya%20A%20%C2%A6%20Naalayak%20%C2%A6%20Ronit%20Vinta%20(152kbit_Opus).mp3",
        "color": "#f97316",
        "id": "308"
    },
    {
        "name": "Akhiyaan Gulaab ; Shahid Kapoor, Kriti Sanon ¦ Mitraz ¦ Teri Baaton Mein Aisa Uljha Jiya",
        "artist": "Unknown Artist",
        "src": "music/Akhiyaan%20Gulaab%20(Song)%3B%20Shahid%20Kapoor%2C%20Kriti%20Sanon%20%C2%A6%20Mitraz%20%C2%A6%20Teri%20Baaton%20Mein%20Aisa%20Uljha%20Jiya%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "19"
    },
    {
        "name": "Alan Walker x King",
        "artist": "Story Of A Bird",
        "src": "music/Story%20Of%20A%20Bird%20-%20Alan%20Walker%20x%20King%20(Live)%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "306"
    },
    {
        "name": "ALBELE TANGE WALE II अलबेले तांगे वाले ll ARUN NAGAR ,ANJALI , SHILPI SINGH II KMC MUSIC",
        "artist": "Unknown Artist",
        "src": "music/ALBELE%20TANGE%20WALE%20II%20%E0%A4%85%E0%A4%B2%E0%A4%AC%E0%A5%87%E0%A4%B2%E0%A5%87%20%E0%A4%A4%E0%A4%BE%E0%A4%82%E0%A4%97%E0%A5%87%20%E0%A4%B5%E0%A4%BE%E0%A4%B2%E0%A5%87%20ll%20ARUN%20NAGAR%20%2CANJALI%20%2C%20SHILPI%20SINGH%20II%20KMC%20MUSIC%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "24"
    },
    {
        "name": "Alekhya Harika ¦ Vinay Shanmukh, Bharatt-Saurabh, Anirudh Ravichander, S Vijay",
        "artist": "Vellake Music",
        "src": "music/Vellake%20Music%20Video%20-%20Alekhya%20Harika%20%C2%A6%20Vinay%20Shanmukh%2C%20Bharatt-Saurabh%2C%20Anirudh%20Ravichander%2C%20S%20Vijay%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "342"
    },
    {
        "name": "All Time Superhit Song - Merupu Kalalu Telugu Movie",
        "artist": "Vennelave Vennelave",
        "src": "music/Vennelave%20Vennelave%20%20-%20%20All%20Time%20Superhit%20Song%20-%20Merupu%20Kalalu%20Telugu%20Movie%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "344"
    },
    {
        "name": "Alone",
        "artist": "Alan Walker",
        "src": "music/Alan%20Walker%20-%20Alone%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "21"
    },
    {
        "name": "Alone, Pt II",
        "artist": "Alan Walker & Ava Max",
        "src": "music/Alan%20Walker%20%26%20Ava%20Max%20-%20Alone%2C%20Pt.%20II%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "20"
    },
    {
        "name": "Andaaz e Karam Madhur Sharma, Moin, ER, Roheb",
        "artist": "Unknown Artist",
        "src": "music/Andaaz%20e%20Karam%20(Official%20Video)%20Madhur%20Sharma%2C%20Moin%2C%20ER%2C%20Roheb%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "26"
    },
    {
        "name": "ANIMAL Ammayi;Ranbir Kapoor,Rashmika M ¦Raghav,PritamAnantha ¦ Sandeep Reddy V¦ Bhushan K",
        "artist": "Unknown Artist",
        "src": "music/ANIMAL%20(Telugu)%20Ammayi%3BRanbir%20Kapoor%2CRashmika%20M%20%C2%A6Raghav%2CPritamAnantha%20%C2%A6%20Sandeep%20Reddy%20V%C2%A6%20Bhushan%20K%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "27"
    },
    {
        "name": "ANIMAL;Evarevaro - ¦ Ranbir K, Rashmika,Anil K, Bobby D ¦ Sandeep V ¦ Vishal Mishra ¦ Bhus…",
        "artist": "Unknown Artist",
        "src": "music/ANIMAL%3BEvarevaro%20-Audio%20%C2%A6%20Ranbir%20K%2C%20Rashmika%2CAnil%20K%2C%20Bobby%20D%20%C2%A6%20Sandeep%20V%20%C2%A6%20Vishal%20Mishra%20%C2%A6%20Bhushan%20K%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "28"
    },
    {
        "name": "Anuvanuvuu Lyrical ¦ Om Bheem Bush ¦ Sree Vishnu¦ Arijit Singh ¦ Harsha Konuganti ¦Sunny M…",
        "artist": "Unknown Artist",
        "src": "music/Anuvanuvuu%20Lyrical%20(Telugu)%20%C2%A6%20Om%20Bheem%20Bush%20%C2%A6%20Sree%20Vishnu%C2%A6%20Arijit%20Singh%20%C2%A6%20Harsha%20Konuganti%20%C2%A6Sunny%20MR%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "31"
    },
    {
        "name": "Arcade ¦ Netherlands 🇳🇱 ¦ Grand Final ¦ Winner of Eurovision 2019",
        "artist": "Duncan Laurence",
        "src": "music/Duncan%20Laurence%20-%20Arcade%20(LIVE)%20%C2%A6%20Netherlands%20%F0%9F%87%B3%F0%9F%87%B1%20%C2%A6%20Grand%20Final%20%C2%A6%20Winner%20of%20Eurovision%202019%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "111"
    },
    {
        "name": "Arerey Manasa ¦ Vishwak Sen, Sid Sriram",
        "artist": "Falaknuma Das",
        "src": "music/Falaknuma%20Das%20-%20Arerey%20Manasa%20Video%20%C2%A6%20Vishwak%20Sen%2C%20Sid%20Sriram%20(152kbit_Opus).mp3",
        "color": "#7c3aed",
        "id": "132"
    },
    {
        "name": "Arz Kiya Hai ¦ Coke Studio Bharat",
        "artist": "Anuv Jain X Lost Stories",
        "src": "music/Anuv%20Jain%20X%20Lost%20Stories%20-%20Arz%20Kiya%20Hai%20(Official%20Video)%20%C2%A6%20Coke%20Studio%20Bharat%20(152kbit_Opus).mp3",
        "color": "#06b6d4",
        "id": "30"
    },
    {
        "name": "Atrangi Re; Rait Zara Si ¦@ARRahman¦Akshay, Dhanush,Sara,Arijit, Shashaa ¦ Bhushan K",
        "artist": "Unknown Artist",
        "src": "music/Atrangi%20Re%3B%20Rait%20Zara%20Si%20Full%20Video%20%C2%A6%40ARRahman%C2%A6Akshay%2C%20Dhanush%2CSara%2CArijit%2C%20Shashaa%20%C2%A6%20Bhushan%20K%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "41"
    },
    {
        "name": "Attention",
        "artist": "Charlie Puth",
        "src": "music/Charlie%20Puth%20-%20Attention%20%5BOfficial%20Video%5D%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "67"
    },
    {
        "name": "Avvy Sra ¦ Karan Aujla ¦ Jaani ¦ Amanninder Singh ¦ 8K",
        "artist": "White Brown Black",
        "src": "music/White%20Brown%20Black%20(Lyrical%20Video)%20-%20Avvy%20Sra%20%C2%A6%20Karan%20Aujla%20%C2%A6%20Jaani%20%C2%A6%20Amanninder%20Singh%20%C2%A6%208K%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "349"
    },
    {
        "name": "Axel F",
        "artist": "Crazy Frog",
        "src": "music/Crazy%20Frog%20-%20Axel%20F%20(Official%20Video)%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "79"
    },
    {
        "name": "Ayudha Pooja Promo ¦ Devara ¦ NTR ¦ Saif Ali Khan ¦ Koratala Siva ¦ Anirudh ¦ Sep 27",
        "artist": "Unknown Artist",
        "src": "music/Ayudha%20Pooja%20Promo%20(Hindi)%20%C2%A6%20Devara%20%C2%A6%20NTR%20%C2%A6%20Saif%20Ali%20Khan%20%C2%A6%20Koratala%20Siva%20%C2%A6%20Anirudh%20%C2%A6%20Sep%2027%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "43"
    },
    {
        "name": "Baby Doll Song Ragini MMS 2 ¦ Sunny Leone ¦ Meet Bros Anjjan feat Kanika Kapoor",
        "artist": "Unknown Artist",
        "src": "music/Baby%20Doll%20Full%20Video%20Song%20Ragini%20MMS%202%20%C2%A6%20Sunny%20Leone%20%C2%A6%20Meet%20Bros%20Anjjan%20Feat.%20Kanika%20Kapoor%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "44"
    },
    {
        "name": "Backbone ¦ Jaani ¦ B Praak ¦ Zenith Sidhu ¦ Music",
        "artist": "Harrdy Sandhu",
        "src": "music/Harrdy%20Sandhu%20-%20Backbone%20%C2%A6%20Jaani%20%C2%A6%20B%20Praak%20%C2%A6%20Zenith%20Sidhu%20%C2%A6%20Official%20Music%20Video%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "147"
    },
    {
        "name": "Bad Boyz",
        "artist": "Unknown Artist",
        "src": "music/Bad%20Boyz%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "45"
    },
    {
        "name": "Baller",
        "artist": "Shubh",
        "src": "music/Shubh%20-%20Baller%20(Official%20Music%20Video)%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "294"
    },
    {
        "name": "Bandana",
        "artist": "Unknown Artist",
        "src": "music/Bandana%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "48"
    },
    {
        "name": "BEAT MÁGICO QUE MEXE COM A SUA MENTE",
        "artist": "Unknown Artist",
        "src": "music/BEAT%20M%C3%81GICO%20QUE%20MEXE%20COM%20A%20SUA%20MENTE%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "51"
    },
    {
        "name": "Beautiful Things",
        "artist": "Benson Boone",
        "src": "music/Benson%20Boone%20-%20Beautiful%20Things%20(Official%20Music%20Video)%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "53"
    },
    {
        "name": "Bekhayali ¦ Kabir Singh ¦ Shahid K,Kiara A¦Sandeep Reddy Vanga ¦ Sachet-Parampara ¦ Irshad",
        "artist": "Unknown Artist",
        "src": "music/Bekhayali%20Full%20Song%20%C2%A6%20Kabir%20Singh%20%C2%A6%20Shahid%20K%2CKiara%20A%C2%A6Sandeep%20Reddy%20Vanga%20%C2%A6%20Sachet-Parampara%20%C2%A6%20Irshad%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "52"
    },
    {
        "name": "Believer",
        "artist": "Imagine Dragons",
        "src": "music/Imagine%20Dragons%20-%20Believer%20(Official%20Music%20Video)%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "167"
    },
    {
        "name": "Bhairava Anthem ¦ Kalki 2898 AD ¦ Diljit Dosanjh ¦ Prabhas ¦ Santhosh Narayanan, Vijaynara…",
        "artist": "Unknown Artist",
        "src": "music/Bhairava%20Anthem%20%C2%A6%20Kalki%202898%20AD%20%C2%A6%20Diljit%20Dosanjh%20%C2%A6%20Prabhas%20%C2%A6%20Santhosh%20Narayanan%2C%20Vijaynarain%2C%20Kumaar%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "54"
    },
    {
        "name": "Bhediya ¦ Varun Dhawan, Kriti Sanon¦ Sachin-Jigar, Arijit Singh, Amitabh Bhattacharya",
        "artist": "Apna Bana Le",
        "src": "music/Apna%20Bana%20Le%20-%20Bhediya%20%C2%A6%20Varun%20Dhawan%2C%20Kriti%20Sanon%C2%A6%20Sachin-Jigar%2C%20Arijit%20Singh%2C%20Amitabh%20Bhattacharya%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "33"
    },
    {
        "name": "BIBI PHONK BR",
        "artist": "Bibi Babydoll",
        "src": "music/Bibi%20Babydoll%20-%20BIBI%20PHONK%20BR%20(Prod.%20DJ%20FKU)%20(152kbit_Opus).mp3",
        "color": "#22c55e",
        "id": "55"
    },
    {
        "name": "Bijlee Bijlee feat Palak Tiwari ¦ Jaani ¦ BPraak ¦ Arvindr Khaira ¦ Desi Melodies",
        "artist": "Harrdy Sandhu",
        "src": "music/Harrdy%20Sandhu%20-%20Bijlee%20Bijlee%20ft%20Palak%20Tiwari%20%C2%A6%20Jaani%20%C2%A6%20BPraak%20%C2%A6%20Arvindr%20Khaira%20%C2%A6%20Desi%20Melodies%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "148"
    },
    {
        "name": "Bilionera",
        "artist": "Otilia",
        "src": "music/Otilia%20-%20Bilionera%20(official%20video)%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "253"
    },
    {
        "name": "Blah Blah Blah",
        "artist": "Armin van Buuren",
        "src": "music/Armin%20van%20Buuren%20-%20Blah%20Blah%20Blah%20(Official%20Lyric%20Video)%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "36"
    },
    {
        "name": "Blinding Lights",
        "artist": "The Weeknd",
        "src": "music/The%20Weeknd%20-%20Blinding%20Lights%20(Official%20Video)%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "320"
    },
    {
        "name": "Blue Eyes Song Yo Yo Honey Singh ¦ Blockbuster Song Of 2013",
        "artist": "Unknown Artist",
        "src": "music/Blue%20Eyes%20Full%20Video%20Song%20Yo%20Yo%20Honey%20Singh%20%C2%A6%20Blockbuster%20Song%20Of%202013%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "57"
    },
    {
        "name": "Body ¦ GRM Daily",
        "artist": "Russ Millions x Tion Wayne",
        "src": "music/Russ%20Millions%20x%20Tion%20Wayne%20-%20Body%20%5BMusic%20Video%5D%20%C2%A6%20GRM%20Daily%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "279"
    },
    {
        "name": "Brahmāstra ¦ Ranbir Kapoor, Alia Bhatt ¦ Pritam ¦ Arijit Singh ¦ Amitabh Bhattacharya¦ 4K",
        "artist": "Kesariya",
        "src": "music/Kesariya%20-%20Brahm%C4%81stra%20%C2%A6%20Ranbir%20Kapoor%2C%20Alia%20Bhatt%20%C2%A6%20Pritam%20%C2%A6%20Arijit%20Singh%20%C2%A6%20Amitabh%20Bhattacharya%C2%A6%204K%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "190"
    },
    {
        "name": "Bujji Thalli Lyrical ¦ Thandel ¦Naga Chaitanya, Sai Pallavi ¦ Javed Ali ¦Shree Mani ¦Devi…",
        "artist": "Unknown Artist",
        "src": "music/Bujji%20Thalli%20Lyrical%20%C2%A6%20Thandel%20%C2%A6Naga%20Chaitanya%2C%20Sai%20Pallavi%20%C2%A6%20Javed%20Ali%20%C2%A6Shree%20Mani%20%C2%A6Devi%20Sri%20Prasad%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "60"
    },
    {
        "name": "By My Side",
        "artist": "AP Dhillon & Shinda Kahlon",
        "src": "music/AP%20Dhillon%20%26%20Shinda%20Kahlon%20-%20By%20My%20Side%20(Official%20Audio)%20(152kbit_Opus).mp3",
        "color": "#ef4444",
        "id": "32"
    },
    {
        "name": "C'Est La Vie",
        "artist": "Khaled",
        "src": "music/Khaled%20-%20C'Est%20La%20Vie%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "191"
    },
    {
        "name": "California Love Cheema Y ¦ Gur Sidhu ¦ Punjabi Song",
        "artist": "Unknown Artist",
        "src": "music/California%20Love%20(Official%20Audio)%20Cheema%20Y%20%C2%A6%20Gur%20Sidhu%20%C2%A6%20Punjabi%20Song%20(152kbit_Opus).mp3",
        "color": "#fb7185",
        "id": "62"
    },
    {
        "name": "Charminar chowrasta kada dj song⁄⁄ #trending ##instagram viral #shorts #2023dj",
        "artist": "Unknown Artist",
        "src": "music/Charminar%20chowrasta%20kada%20dj%20song%E2%81%84%E2%81%84%20%23trending%20%23%23instagram%20viral%20%23shorts%20%232023dj%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "69"
    },
    {
        "name": "Cheap Thrills",
        "artist": "Sia",
        "src": "music/Sia%20-%20Cheap%20Thrills%20(Performance%20Edit)%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "298"
    },
    {
        "name": "Cheap Thrills feat Sean Paul",
        "artist": "Sia",
        "src": "music/Sia%20-%20Cheap%20Thrills%20(Lyrics)%20ft.%20Sean%20Paul%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "297"
    },
    {
        "name": "Check It Out",
        "artist": "Parmish Verma feat Paradox",
        "src": "music/Parmish%20Verma%20Ft.%20Paradox%20-%20Check%20It%20Out%20(Official%20Music%20Video)%20(152kbit_Opus).mp3",
        "color": "#fb7185",
        "id": "256"
    },
    {
        "name": "Cheliya",
        "artist": "Unknown Artist",
        "src": "music/Cheliya%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "70"
    },
    {
        "name": "Cheques",
        "artist": "Shubh",
        "src": "music/Shubh%20-%20Cheques%20(Official%20Audio)%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "295"
    },
    {
        "name": "Cheri Cheri Lady",
        "artist": "Modern Talking",
        "src": "music/Modern%20Talking%20-%20Cheri%20Cheri%20Lady%20(Official%C2%A0Video)%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "229"
    },
    {
        "name": "Chinnadhana With Telugu ¦¦'మా పాట మీ నోట'¦¦ Ishq Songs",
        "artist": "Unknown Artist",
        "src": "music/Chinnadhana%20Full%20Song%20With%20Telugu%20Lyrics%20%C2%A6%C2%A6'%E0%B0%AE%E0%B0%BE%20%E0%B0%AA%E0%B0%BE%E0%B0%9F%20%E0%B0%AE%E0%B1%80%20%E0%B0%A8%E0%B1%8B%E0%B0%9F'%C2%A6%C2%A6%20Ishq%20Songs%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "72"
    },
    {
        "name": "Chiru Chiru ¦ Yuvanshankar ¦ Karthi",
        "artist": "Awaara",
        "src": "music/Awaara%20-%20Chiru%20Chiru%20Video%20%C2%A6%20Yuvanshankar%20%C2%A6%20Karthi%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "42"
    },
    {
        "name": "Chuttamalle ¦ Devara Second Single ¦ NTR ¦ Janhvi Kapoor ¦ Anirudh Ravichander ¦ Shilpa Ra…",
        "artist": "Unknown Artist",
        "src": "music/Chuttamalle%20%C2%A6%20Devara%20Second%20Single%20%C2%A6%20NTR%20%C2%A6%20Janhvi%20Kapoor%20%C2%A6%20Anirudh%20Ravichander%20%C2%A6%20Shilpa%20Rao%20%C2%A6%2027%20Sep%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "74"
    },
    {
        "name": "Close To The Sun",
        "artist": "TheFatRat & Anjulie",
        "src": "music/TheFatRat%20%26%20Anjulie%20-%20Close%20To%20The%20Sun%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "325"
    },
    {
        "name": "COMPANY",
        "artist": "EMIWAY",
        "src": "music/EMIWAY%20-%20COMPANY%20(OFFICIAL%20MUSIC%20VIDEO)%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "122"
    },
    {
        "name": "Counting Stars",
        "artist": "OneRepublic",
        "src": "music/OneRepublic%20-%20Counting%20Stars%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "251"
    },
    {
        "name": "Cover - Yohani & Satheeshan",
        "artist": "Manike Mage Hithe මැණිකේ මගේ හිතේ",
        "src": "music/Manike%20Mage%20Hithe%20%20%E0%B6%B8%E0%B7%90%E0%B6%AB%E0%B7%92%E0%B6%9A%E0%B7%9A%20%E0%B6%B8%E0%B6%9C%E0%B7%9A%20%E0%B7%84%E0%B7%92%E0%B6%AD%E0%B7%9A%20%20-%20Official%20Cover%20-%20Yohani%20%26%20Satheeshan%20(152kbit_Opus).mp3",
        "color": "#06b6d4",
        "id": "218"
    },
    {
        "name": "Cradles",
        "artist": "Sub Urban",
        "src": "music/Sub%20Urban%20-%20Cradles%20%5BOfficial%20Music%20Video%5D%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "307"
    },
    {
        "name": "Cuando se moja la tarea",
        "artist": "Unknown Artist",
        "src": "music/Cuando%20se%20moja%20la%20tarea%20(Japanese%20Remix)%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "80"
    },
    {
        "name": "CUTE DEPRESSED",
        "artist": "Dyan Dxddy",
        "src": "music/Dyan%20Dxddy%20-%20CUTE%20DEPRESSED%20(160kbit_Opus).mp3",
        "color": "#7c3aed",
        "id": "113"
    },
    {
        "name": "Da Dumla Dumlada",
        "artist": "Unknown Artist",
        "src": "music/Da%20Dumla%20Dumlada%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "81"
    },
    {
        "name": "Daddy Yankee ¦ Gasolina",
        "artist": "Unknown Artist",
        "src": "music/Daddy%20Yankee%20%C2%A6%20Gasolina%20(Video%20Oficial)%20%5BFull%20HD%5D%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "82"
    },
    {
        "name": "DAKU ¦ INDERPAL MOGA ¦ CHANI NATTAN ¦ NEW PUNJABI SONG ¦ LATEST PUNJABI SONG 2022 ¦Daku Ik…",
        "artist": "Unknown Artist",
        "src": "music/DAKU%20%C2%A6%20INDERPAL%20MOGA%20%C2%A6%20CHANI%20NATTAN%20%C2%A6%20NEW%20PUNJABI%20SONG%20%C2%A6%20LATEST%20PUNJABI%20SONG%202022%20%C2%A6Daku%20Ik%20number%20da%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "83"
    },
    {
        "name": "Damru; Bassick, Karan Kanchan ¦ Mtv Hustle Season 3 REPRESENT ¦ Hustle 3 0",
        "artist": "Unknown Artist",
        "src": "music/Damru%3B%20Bassick%2C%20Karan%20Kanchan%20%C2%A6%20Mtv%20Hustle%20Season%203%20REPRESENT%20%C2%A6%20Hustle%203.0%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "84"
    },
    {
        "name": "dança zip",
        "artist": "Ariis",
        "src": "music/Ariis%20-%20dan%C3%A7a%20zip%20%5BBrazilian%20Phonk%5D%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "35"
    },
    {
        "name": "Dandelions",
        "artist": "Ruth B",
        "src": "music/Ruth%20B.%20-%20Dandelions%20(Lyrics)%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "280"
    },
    {
        "name": "Dandelions",
        "artist": "Ruth B",
        "src": "music/Ruth%20B.%20-%20Dandelions%20(Lyrics)%20(Slowed%20%2B%20Reverb)%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "281"
    },
    {
        "name": "Darkside",
        "artist": "Alan Walker",
        "src": "music/Alan%20Walker%20-%20Darkside%20(feat.%20Au%E2%81%84Ra%20and%20Tomine%20Harket)%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "22"
    },
    {
        "name": "Darkside",
        "artist": "NEONI",
        "src": "music/NEONI%20-%20Darkside%20(Lyrics)%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "239"
    },
    {
        "name": "Dastaan ¦ Denny x Rahul Mishra ¦ Kunaal Vermaa",
        "artist": "Unknown Artist",
        "src": "music/Dastaan%20(Official%20Video)%20%C2%A6%20Denny%20x%20Rahul%20Mishra%20%C2%A6%20Kunaal%20Vermaa%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "85"
    },
    {
        "name": "De De Pyaar De 2 ¦ Ajay, Rakul, Meezaan ¦ Payal D, Aditya D, Aditya R, Kumaar ¦ 14th Nov",
        "artist": "Raat Bhar",
        "src": "music/Raat%20Bhar%20%E2%80%93%20De%20De%20Pyaar%20De%202%20%C2%A6%20Ajay%2C%20Rakul%2C%20Meezaan%20%C2%A6%20Payal%20D%2C%20Aditya%20D%2C%20Aditya%20R%2C%20Kumaar%20%C2%A6%2014th%20Nov%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "268"
    },
    {
        "name": "Deeper Than It Seems",
        "artist": "Jace June",
        "src": "music/Jace%20June%20-%20Deeper%20Than%20It%20Seems%20(Visualizer)%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "177"
    },
    {
        "name": "DEMONS IN MY SOUL",
        "artist": "Unknown Artist",
        "src": "music/DEMONS%20IN%20MY%20SOUL%20(160kbit_Opus).mp3",
        "color": "#7c3aed",
        "id": "88"
    },
    {
        "name": "Dernière Danse",
        "artist": "Indila",
        "src": "music/Indila%20-%20Derni%C3%A8re%20Danse%20(Clip%20Officiel)%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "171"
    },
    {
        "name": "Desabafo Deixa Eu Dizer",
        "artist": "Marcelo D2 Claudia",
        "src": "music/Marcelo%20D2%20Claudia%20-%20Desabafo%20Deixa%20Eu%20Dizer%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "220"
    },
    {
        "name": "Despacito feat Daddy Yankee",
        "artist": "Luis Fonsi",
        "src": "music/Luis%20Fonsi%20-%20Despacito%20ft.%20Daddy%20Yankee%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "212"
    },
    {
        "name": "Deva Deva",
        "artist": "Unknown Artist",
        "src": "music/Deva%20Deva%20(From%20'Brahmastra')%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "90"
    },
    {
        "name": "Devi Sri Prasad Sings Oo Antava Song As Devotional Song ¦ Hilarious Fun ¦ Manastars",
        "artist": "Unknown Artist",
        "src": "music/Devi%20Sri%20Prasad%20Sings%20Oo%20Antava%20Song%20As%20Devotional%20Song%20%C2%A6%20Hilarious%20Fun%20%C2%A6%20Manastars%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "91"
    },
    {
        "name": "Devil ¦ Trap ¦ NCS - Copyright Free Music",
        "artist": "Barren Gates",
        "src": "music/Barren%20Gates%20-%20Devil%20%C2%A6%20Trap%20%C2%A6%20NCS%20-%20Copyright%20Free%20Music%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "49"
    },
    {
        "name": "Devil Eyes",
        "artist": "Hippie Sabotage",
        "src": "music/Hippie%20Sabotage%20-%20Devil%20Eyes%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "156"
    },
    {
        "name": "Dheere Dheere ¦ Devara ¦ NTR ¦ Janhvi Kapoor ¦ Anirudh ¦ Shilpa Rao",
        "artist": "Unknown Artist",
        "src": "music/Dheere%20Dheere%20(Full%20Video)%20%C2%A6%20Devara%20%C2%A6%20NTR%20%C2%A6%20Janhvi%20Kapoor%20%C2%A6%20Anirudh%20%C2%A6%20Shilpa%20Rao%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "95"
    },
    {
        "name": "DILBAR Lyrical ¦ Satyameva Jayate ¦John Abraham, Nora Fatehi,Tanishk B, Neha Kakkar,Dhvani…",
        "artist": "Unknown Artist",
        "src": "music/DILBAR%20Lyrical%20%C2%A6%20Satyameva%20Jayate%20%C2%A6John%20Abraham%2C%20Nora%20Fatehi%2CTanishk%20B%2C%20Neha%20Kakkar%2CDhvani%2C%20Ikka%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "97"
    },
    {
        "name": "Diljit Dosanjh; Born To Shine G O A T",
        "artist": "Unknown Artist",
        "src": "music/Diljit%20Dosanjh%3B%20Born%20To%20Shine%20(Official%20Music%20Video)%20G.O.A.T%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "98"
    },
    {
        "name": "Diljit Dosanjh; Kinni Kinni GHOST ¦ Thiarajxtt, Raj Ranjodh",
        "artist": "Unknown Artist",
        "src": "music/Diljit%20Dosanjh%3B%20Kinni%20Kinni%20(Official%20Audio)%20GHOST%20%C2%A6%20Thiarajxtt%2C%20Raj%20Ranjodh%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "99"
    },
    {
        "name": "Diljit Dosanjh; LOVER Intense ¦ Raj Ranjodh ¦ MoonChild Era",
        "artist": "Unknown Artist",
        "src": "music/Diljit%20Dosanjh%3B%20LOVER%20(Official%20Music%20Video)%20Intense%20%C2%A6%20Raj%20Ranjodh%20%C2%A6%20MoonChild%20Era%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "100"
    },
    {
        "name": "Diwali Deepaanni Song ¦¦ Dhada Songs ¦¦ Naga Chaitnya, Kajal Agarwal",
        "artist": "Unknown Artist",
        "src": "music/Diwali%20Deepaanni%20Video%20Song%20%C2%A6%C2%A6%20Dhada%20Video%20Songs%20%C2%A6%C2%A6%20Naga%20Chaitnya%2C%20Kajal%20Agarwal%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "101"
    },
    {
        "name": "Do Joru Vaala",
        "artist": "Unknown Artist",
        "src": "music/Do%20Joru%20Vaala%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "106"
    },
    {
        "name": "Dookudu ; Poovai Poovai",
        "artist": "Unknown Artist",
        "src": "music/Dookudu%20%3B%20Poovai%20Poovai%20HD%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "109"
    },
    {
        "name": "Dreams pt II ¦ Trap ¦ NCS - Copyright Free Music",
        "artist": "Lost Sky",
        "src": "music/Lost%20Sky%20-%20Dreams%20pt.%20II%20(feat.%20Sara%20Skinner)%20%C2%A6%20Trap%20%C2%A6%20NCS%20-%20Copyright%20Free%20Music%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "208"
    },
    {
        "name": "Drive forever",
        "artist": "Unknown Artist",
        "src": "music/Drive%20forever%20(Remix)%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "110"
    },
    {
        "name": "Dunki Drop 5; O Maahi ¦ Shah Rukh Khan ¦ Taapsee Pannu ¦ Pritam ¦ Arijit Singh ¦ Irshad Ka…",
        "artist": "Unknown Artist",
        "src": "music/Dunki%20Drop%205%3B%20O%20Maahi%20%C2%A6%20Shah%20Rukh%20Khan%20%C2%A6%20Taapsee%20Pannu%20%C2%A6%20Pritam%20%C2%A6%20Arijit%20Singh%20%C2%A6%20Irshad%20Kamil%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "112"
    },
    {
        "name": "Ee Chota Nuvvunna ¦Johnny¦Pawan Kalyan¦Pawan Kalyan,Ramana Gogula Hits ¦ Aditya Music",
        "artist": "Unknown Artist",
        "src": "music/Ee%20Chota%20Nuvvunna%20Full%20Song%20%C2%A6Johnny%C2%A6Pawan%20Kalyan%C2%A6Pawan%20Kalyan%2CRamana%20Gogula%20Hits%20%C2%A6%20Aditya%20Music%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "116"
    },
    {
        "name": "EITA NOVINHA DELICIA, ELA TÁ DE MARQUINHA DE FITA vs FUNK TIK TOK ¦ MC PÂNICO & DJ VT DA R…",
        "artist": "Unknown Artist",
        "src": "music/EITA%20NOVINHA%20DELICIA%2C%20ELA%20T%C3%81%20DE%20MARQUINHA%20DE%20FITA%20vs%20FUNK%20TIK%20TOK%20%20%C2%A6%20MC%20P%C3%82NICO%20%26%20DJ%20VT%20DA%20RUA%20LARGA%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "117"
    },
    {
        "name": "Emo Emo Emo Song ¦¦ Raahu Movie ¦¦ Sid Sriram ¦¦ Praveen Lakkaraju ¦¦ Subbu Vedula ¦¦ Aber…",
        "artist": "Unknown Artist",
        "src": "music/Emo%20Emo%20Emo%20Video%20Song%20%C2%A6%C2%A6%20Raahu%20Movie%20%C2%A6%C2%A6%20Sid%20Sriram%20%C2%A6%C2%A6%20Praveen%20Lakkaraju%20%C2%A6%C2%A6%20Subbu%20Vedula%20%C2%A6%C2%A6%20Aberaam%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "123"
    },
    {
        "name": "endhuko emo live performance by alap raju MPG",
        "artist": "Unknown Artist",
        "src": "music/endhuko%20emo%20live%20performance%20by%20alap%20raju.MPG%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "124"
    },
    {
        "name": "Enna Sona ¦ OK Jaanu ¦ Arijit Singh ¦ Shraddha Kapoor ¦ Aditya Roy Kapoor",
        "artist": "Unknown Artist",
        "src": "music/Enna%20Sona%20(Official%20Video)%20%C2%A6%20OK%20Jaanu%20%C2%A6%20Arijit%20Singh%20%C2%A6%20Shraddha%20Kapoor%20%C2%A6%20Aditya%20Roy%20Kapoor%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "125"
    },
    {
        "name": "Enna Sona ¦ OK Jaanu ¦ Arijit Singh ¦ Shraddha Kapoor ¦ Aditya Roy Kapoor",
        "artist": "Unknown Artist",
        "src": "music/Enna%20Sona%20(Official%20Video)%20%C2%A6%20OK%20Jaanu%20%C2%A6%20Arijit%20Singh%20%C2%A6%20Shraddha%20Kapoor%20%C2%A6%20Aditya%20Roy%20Kapoor%20(152kbit_Opus).mp3",
        "color": "#f97316",
        "id": "126"
    },
    {
        "name": "ENTÃO SALVA NA 🇪🇸 x VOU TE PASSA A BALINHA -MC NEGÃO VHF",
        "artist": "Unknown Artist",
        "src": "music/ENT%C3%83O%20SALVA%20NA%20%F0%9F%87%AA%F0%9F%87%B8%20x%20VOU%20TE%20PASSA%20A%20BALINHA%20-MC%20NEG%C3%83O%20VHF(%20DJ%20R9%20DA)%20(160kbit_Opus).mp3",
        "color": "#22c55e",
        "id": "127"
    },
    {
        "name": "Excuses ¦ AP Dhillon ¦ Gurinder Gill ¦ Intense",
        "artist": "Unknown Artist",
        "src": "music/Excuses%20(Official%20Video)%20%C2%A6%20AP%20Dhillon%20%C2%A6%20Gurinder%20Gill%20%C2%A6%20Intense%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "130"
    },
    {
        "name": "Faded",
        "artist": "Alan Walker",
        "src": "music/Alan%20Walker%20-%20Faded%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "23"
    },
    {
        "name": "Falak Tak Song ¦ Tashan ¦ Akshay Kumar, Kareena Kapoor, Udit Narayan, Mahalaxmi Iyer, Vish…",
        "artist": "Unknown Artist",
        "src": "music/Falak%20Tak%20Song%20%C2%A6%20Tashan%20%C2%A6%20Akshay%20Kumar%2C%20Kareena%20Kapoor%2C%20Udit%20Narayan%2C%20Mahalaxmi%20Iyer%2C%20Vishal-Shekhar%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "131"
    },
    {
        "name": "FAREBI ¦ Pyaar Diwaari Arc 3",
        "artist": "Chaar Diwaari X @raftaarmusic",
        "src": "music/Chaar%20Diwaari%20X%20%40raftaarmusic%20%20-%20FAREBI%20(Official%20Video)%20%C2%A6%20Pyaar%20Diwaari%20Arc%203%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "66"
    },
    {
        "name": "Father Stretch My Hands Pt 1",
        "artist": "Unknown Artist",
        "src": "music/Father%20Stretch%20My%20Hands%20Pt.%201%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "133"
    },
    {
        "name": "Favorite",
        "artist": "Isabel LaRosa",
        "src": "music/Isabel%20LaRosa%20-%20Favorite%20(Lyric%20Video)%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "173"
    },
    {
        "name": "FE!N feat Playboi Carti",
        "artist": "Travis Scott",
        "src": "music/Travis%20Scott%20-%20FE!N%20ft.%20Playboi%20Carti%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "330"
    },
    {
        "name": "Fearless pt II ¦ Trap ¦ NCS - Copyright Free Music",
        "artist": "Lost Sky",
        "src": "music/Lost%20Sky%20-%20Fearless%20pt.II%20(feat.%20Chris%20Linton)%20%C2%A6%20Trap%20%C2%A6%20NCS%20-%20Copyright%20Free%20Music%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "209"
    },
    {
        "name": "Fed Up",
        "artist": "Bazanji",
        "src": "music/Bazanji%20-%20Fed%20Up%20(Official%20Music%20Video)%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "50"
    },
    {
        "name": "FUNKED UP",
        "artist": "Unknown Artist",
        "src": "music/FUNKED%20UP%20(SLOWED)%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "139"
    },
    {
        "name": "Gangsta's Paradise",
        "artist": "Coolio",
        "src": "music/Coolio%20-%20Gangsta's%20Paradise%20(feat.%20L.V.)%20%5BOfficial%20Music%20Video%5D%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "78"
    },
    {
        "name": "Genda Phool ¦ Jacqueline Fernandez ¦ Payal Dev ¦ Hit Anthem of the Year 2021",
        "artist": "Badshah",
        "src": "music/Badshah%20%E2%80%93%20Genda%20Phool%20%C2%A6%20Jacqueline%20Fernandez%20%C2%A6%20Payal%20Dev%20%C2%A6%20Hit%20Anthem%20of%20the%20Year%202021%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "46"
    },
    {
        "name": "Genius feat Sia, Diplo, Labrinth",
        "artist": "LSD",
        "src": "music/LSD%20-%20Genius%20ft.%20Sia%2C%20Diplo%2C%20Labrinth%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "210"
    },
    {
        "name": "GigaChad Theme",
        "artist": "g3ox em",
        "src": "music/g3ox_em%20-%20GigaChad%20Theme%20(Phonk%20House%20Version)%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "140"
    },
    {
        "name": "Go Down Deh ¦ Music",
        "artist": "Spice, Sean Paul, Shaggy",
        "src": "music/Spice%2C%20Sean%20Paul%2C%20Shaggy%20-%20Go%20Down%20Deh%20%C2%A6%20Official%20Music%20Video%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "305"
    },
    {
        "name": "goosebumps feat Kendrick Lamar",
        "artist": "Travis Scott",
        "src": "music/Travis%20Scott%20-%20goosebumps%20(Official%20Video)%20ft.%20Kendrick%20Lamar%20(160kbit_Opus).mp3",
        "color": "#fb7185",
        "id": "331"
    },
    {
        "name": "Gowliguda Gunshot Jetty Barath Yadav",
        "artist": "Unknown Artist",
        "src": "music/Gowliguda%20Gunshot%20Jetty%20Barath%20Yadav%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "142"
    },
    {
        "name": "Guruvaram ¦ Amrithamayi ¦ Rahul Nambiar ¦ Telugu ¦ Malayalam",
        "artist": "Unknown Artist",
        "src": "music/Guruvaram%20%C2%A6%20Amrithamayi%20%C2%A6%20Rahul%20Nambiar%20%C2%A6%20Telugu%20%C2%A6%20Malayalam%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "144"
    },
    {
        "name": "Gym Class Heroes; Stereo Hearts feat Adam Levine",
        "artist": "Unknown Artist",
        "src": "music/Gym%20Class%20Heroes%3B%20Stereo%20Hearts%20ft.%20Adam%20Levine%20%5BOFFICIAL%20VIDEO%5D%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "145"
    },
    {
        "name": "Habibi",
        "artist": "Ricky Rich & ARAM Mafia",
        "src": "music/Ricky%20Rich%20%26%20ARAM%20Mafia%20-%20Habibi%20(Official%20Video)%20(152kbit_Opus).mp3",
        "color": "#ef4444",
        "id": "277"
    },
    {
        "name": "Halamithi Habibo",
        "artist": "Arabic Kuthu",
        "src": "music/Arabic%20Kuthu%20-%20Halamithi%20Habibo%20(From%20'Beast')%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "34"
    },
    {
        "name": "Hanumankind - Big Dawgs ¦ Prod Kalmi ¦ Def Jam India",
        "artist": "Hanumankind, Kalmi",
        "src": "music/Hanumankind%2C%20Kalmi%20-%20Hanumankind%20%E2%80%93%20Big%20Dawgs%20%C2%A6%20Prod.%20Kalmi%20(Official%20Music%20Video)%20%C2%A6%20Def%20Jam%20India%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "146"
    },
    {
        "name": "Haryana Hood Slow + Reverb",
        "artist": "Unknown Artist",
        "src": "music/Haryana%20Hood%20Slow%20%2B%20Reverb%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "150"
    },
    {
        "name": "Hass Hass Diljit X Sia",
        "artist": "Unknown Artist",
        "src": "music/Hass%20Hass%20(Official%20Video)%20Diljit%20X%20Sia%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "151"
    },
    {
        "name": "Havana feat Young Thug",
        "artist": "Camila Cabello",
        "src": "music/Camila%20Cabello%20-%20Havana%20(Official%20Video)%20ft.%20Young%20Thug%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "63"
    },
    {
        "name": "Heat Waves",
        "artist": "Glass Animals",
        "src": "music/Glass%20Animals%20-%20Heat%20Waves%20(Official%20Video)%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "141"
    },
    {
        "name": "Heeriye Jasleen Royal feat Arijit Singh¦ Dulquer Salmaan¦ Aditya Sharma ¦Taani Tanvir",
        "artist": "Unknown Artist",
        "src": "music/Heeriye%20(Official%20Video)%20Jasleen%20Royal%20ft%20Arijit%20Singh%C2%A6%20Dulquer%20Salmaan%C2%A6%20Aditya%20Sharma%20%C2%A6Taani%20Tanvir%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "153"
    },
    {
        "name": "Hey Mama feat Nicki Minaj, Bebe Rexha & Afrojack",
        "artist": "David Guetta",
        "src": "music/David%20Guetta%20-%20Hey%20Mama%20(Official%20Video)%20ft%20Nicki%20Minaj%2C%20Bebe%20Rexha%20%26%20Afrojack%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "86"
    },
    {
        "name": "High Rated Gabru",
        "artist": "Unknown Artist",
        "src": "music/High%20Rated%20Gabru%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "154"
    },
    {
        "name": "Hilesso Hilessa Lyrical ¦ Thandel ¦ Naga Chaitanya, Sai Pallavi ¦Nakash Aziz, Shreya Ghosh…",
        "artist": "Unknown Artist",
        "src": "music/Hilesso%20Hilessa%20Lyrical%20%C2%A6%20Thandel%20%C2%A6%20Naga%20Chaitanya%2C%20Sai%20Pallavi%20%C2%A6Nakash%20Aziz%2C%20Shreya%20Ghoshal%20%C2%A6%20DSP%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "155"
    },
    {
        "name": "Hope",
        "artist": "Unknown Artist",
        "src": "music/Hope%20(152kbit_Opus).mp3",
        "color": "#22c55e",
        "id": "157"
    },
    {
        "name": "Hurts So Good",
        "artist": "Astrid S",
        "src": "music/Astrid%20S%20-%20Hurts%20So%20Good%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "40"
    },
    {
        "name": "HYDERABADI MARFA ¦ DJ NIKHIL MARTYN",
        "artist": "Unknown Artist",
        "src": "music/HYDERABADI%20MARFA%20%C2%A6%20DJ%20NIKHIL%20MARTYN%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "160"
    },
    {
        "name": "Hymn For The Weekend",
        "artist": "Coldplay",
        "src": "music/Coldplay%20-%20Hymn%20For%20The%20Weekend%20(Official%20Video)%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "77"
    },
    {
        "name": "I Ain’t Worried",
        "artist": "OneRepublic",
        "src": "music/OneRepublic%20-%20I%20Ain%E2%80%99t%20Worried%20(From%20%E2%80%9CTop%20Gun%3B%20Maverick%E2%80%9D)%20%5BOfficial%20Music%20Video%5D%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "252"
    },
    {
        "name": "I Wanna Be Yours",
        "artist": "Unknown Artist",
        "src": "music/I%20Wanna%20Be%20Yours%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "162"
    },
    {
        "name": "I'm an Albatraoz ¦",
        "artist": "AronChupa, Little Sis Nora",
        "src": "music/AronChupa%2C%20Little%20Sis%20Nora%20-%20I'm%20an%20Albatraoz%20%C2%A6%20OFFICIAL%20VIDEO%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "37"
    },
    {
        "name": "I'm Done ¦ Maan Panu ¦ I-Popstar ; Vol 1 ¦ EP02 ¦ Amazon MX Player",
        "artist": "Unknown Artist",
        "src": "music/I'm%20Done%20%C2%A6%20Maan%20Panu%20%C2%A6%20I-Popstar%20%3B%20Vol.1%20%C2%A6%20EP02%20%C2%A6%20Amazon%20MX%20Player%20(152kbit_Opus).mp3",
        "color": "#22c55e",
        "id": "163"
    },
    {
        "name": "Idhedho Bagundhe Cheli Song ¦ Vijay Prakash,Sunitha Performance ¦ Super Masti ¦Ongole ¦7th…",
        "artist": "Unknown Artist",
        "src": "music/Idhedho%20Bagundhe%20Cheli%20Song%20%C2%A6%20Vijay%20Prakash%2CSunitha%20Performance%20%C2%A6%20Super%20Masti%20%C2%A6Ongole%20%C2%A67th%20May%202017%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "164"
    },
    {
        "name": "Ignite",
        "artist": "K-391 & Alan Walker",
        "src": "music/K-391%20%26%20Alan%20Walker%20-%20Ignite%20(feat.%20Julie%20Bergan%20%26%20Seungri)%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "183"
    },
    {
        "name": "Iktara",
        "artist": "Unknown Artist",
        "src": "music/Iktara%20(MTV%20Unplugged%20Version)%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "165"
    },
    {
        "name": "Ilahi Song ¦ Yeh Jawaani Hai Deewani ¦ Ranbir Kapoor, Deepika Padukone ¦ Pritam",
        "artist": "Unknown Artist",
        "src": "music/Ilahi%20Full%20Video%20Song%20%C2%A6%20Yeh%20Jawaani%20Hai%20Deewani%20%C2%A6%20Ranbir%20Kapoor%2C%20Deepika%20Padukone%20%C2%A6%20Pritam%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "166"
    },
    {
        "name": "Indila-Ainsi bas la vida",
        "artist": "Unknown Artist",
        "src": "music/Indila-Ainsi%20bas%20la%20vida%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "172"
    },
    {
        "name": "Industry Baby feat Jack Harlow",
        "artist": "Lil Nas X",
        "src": "music/Lil%20Nas%20X%20-%20Industry%20Baby%20(Lyrics)%20ft.%20Jack%20Harlow%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "204"
    },
    {
        "name": "Invisible ¦ Trap ¦ NCS - Copyright Free Music",
        "artist": "Julius Dreisig & Zeus X Crona",
        "src": "music/Julius%20Dreisig%20%26%20Zeus%20X%20Crona%20-%20Invisible%20%C2%A6%20Trap%20%C2%A6%20NCS%20-%20Copyright%20Free%20Music%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "182"
    },
    {
        "name": "Ishq",
        "artist": "Unknown Artist",
        "src": "music/Ishq%20(From%20'Lost%3BFound')%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "174"
    },
    {
        "name": "Jab Harry Met Sejal ¦ Shah Rukh Khan, Anushka ¦ Arijit Singh ¦ Pritam",
        "artist": "Hawayein Lyrical",
        "src": "music/Hawayein%20Lyrical%20Video%20-%20Jab%20Harry%20Met%20Sejal%20%C2%A6%20Shah%20Rukh%20Khan%2C%20Anushka%20%C2%A6%20Arijit%20Singh%20%C2%A6%20Pritam%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "152"
    },
    {
        "name": "Jalebi Baby",
        "artist": "Tesher x Jason Derulo",
        "src": "music/Tesher%20x%20Jason%20Derulo%20-%20Jalebi%20Baby%20(Official%20Video)%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "318"
    },
    {
        "name": "Jhol ¦ Coke Studio Pakistan ¦ Season 15 ¦ Maanu x Annural Khalid",
        "artist": "Unknown Artist",
        "src": "music/Jhol%20%C2%A6%20Coke%20Studio%20Pakistan%20%C2%A6%20Season%2015%20%C2%A6%20Maanu%20x%20Annural%20Khalid%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "180"
    },
    {
        "name": "Jhoom Sharaabi – De De Pyaar De 2 ¦ Ajay Devgn, Rakul Preet Singh ¦ Yo Yo Honey Singh ¦ 14…",
        "artist": "Unknown Artist",
        "src": "music/Jhoom%20Sharaabi%20(Song)%E2%80%93%20De%20De%20Pyaar%20De%202%20%C2%A6%20Ajay%20Devgn%2C%20Rakul%20Preet%20Singh%20%C2%A6%20Yo%20Yo%20Honey%20Singh%20%C2%A6%2014%20Nov%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "181"
    },
    {
        "name": "JO TUM MERE HO",
        "artist": "Anuv Jain",
        "src": "music/Anuv%20Jain%20-%20JO%20TUM%20MERE%20HO%20(Official%20Video)%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "29"
    },
    {
        "name": "JUJALARIM FUNK",
        "artist": "Eternxlkz",
        "src": "music/Eternxlkz%20-%20JUJALARIM%20FUNK%20(Official%20Audio)%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "128"
    },
    {
        "name": "KAATON ; JACK LOVE ¦ LUV SHARMA ¦ GXZ ¦ PUNJABI SONG",
        "artist": "Unknown Artist",
        "src": "music/KAATON%20%3B%20JACK%20LOVE%20%C2%A6%20LUV%20SHARMA%20%C2%A6%20GXZ%20%C2%A6%20PUNJABI%20SONG%20(%20LYRICAL%20VIDEO%20)%20(152kbit_Opus).mp3",
        "color": "#fb7185",
        "id": "184"
    },
    {
        "name": "Kabhi Jo Badal Barse Arijit Singh",
        "artist": "Unknown Artist",
        "src": "music/Kabhi%20Jo%20Badal%20Barse%20Lyrics%20%20Arijit%20Singh%20(Jackpot)%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "185"
    },
    {
        "name": "Kailash Kher ¦ ¦ Kailasa ¦ Paresh ¦ Naresh",
        "artist": "Teri Deewani",
        "src": "music/Teri%20Deewani%20-%20Kailash%20Kher%20%C2%A6%20Official%20Video%20%C2%A6%20Kailasa%20%C2%A6%20Paresh%20%C2%A6%20Naresh%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "316"
    },
    {
        "name": "Kailash Kher Live Performance at Isha Yoga Center",
        "artist": "Bam Lahiri",
        "src": "music/Bam%20Lahiri%20-%20Kailash%20Kher%20Live%20Performance%20at%20Isha%20Yoga%20Center%20(MahaShivRatri%202017)%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "47"
    },
    {
        "name": "Kalalo Kooda Music ¦ Liger ¦ Vijay Deverakonda, Ananya Panday ¦ Tanishk Bagchi",
        "artist": "Unknown Artist",
        "src": "music/Kalalo%20Kooda%20Music%20Video%20%C2%A6%20Liger%20(Telugu)%20%C2%A6%20Vijay%20Deverakonda%2C%20Ananya%20Panday%20%C2%A6%20Tanishk%20Bagchi%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "187"
    },
    {
        "name": "Kaleshi Chori Pranjal Dahiya ¦ DG IMMORTALS, Raga, Harjas, Virtual AF, Sshiv ¦ Deepesh Goy…",
        "artist": "Unknown Artist",
        "src": "music/Kaleshi%20Chori%20(Video)%20Pranjal%20Dahiya%20%C2%A6%20DG%20IMMORTALS%2C%20Raga%2C%20Harjas%2C%20Virtual_AF%2C%20Sshiv%20%C2%A6%20Deepesh%20Goyal%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "188"
    },
    {
        "name": "Killing Jeeja",
        "artist": "Unknown Artist",
        "src": "music/Killing%20Jeeja%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "193"
    },
    {
        "name": "King Shit",
        "artist": "Shubh",
        "src": "music/Shubh%20-%20King%20Shit%20(Official%20Audio)%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "296"
    },
    {
        "name": "KISSIK Lyrical ¦ Pushpa 2 The Rule ¦ Allu Arjun ¦ Sukumar ¦ Sreeleela ¦ DSP",
        "artist": "Unknown Artist",
        "src": "music/KISSIK%20Lyrical%20Video%20%C2%A6%20Pushpa%202%20The%20Rule%20%C2%A6%20Allu%20Arjun%20%C2%A6%20Sukumar%20%C2%A6%20Sreeleela%20%C2%A6%20DSP%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "194"
    },
    {
        "name": "Knife Brows",
        "artist": "Dhanda Nyoliwala",
        "src": "music/Dhanda%20Nyoliwala%20-%20Knife%20Brows%20(Official%20Audio)%20(152kbit_Opus).mp3",
        "color": "#22c55e",
        "id": "92"
    },
    {
        "name": "KOSANDRA",
        "artist": "MIYAGI × ANDY PANDA",
        "src": "music/MIYAGI%20%C3%97%20ANDY%20PANDA%20-%20KOSANDRA%20(MIKIS%20REMIX)%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "228"
    },
    {
        "name": "Krish Rao ¦ Gully Gang Records ¦ Latest Haryanvi Song 2024",
        "artist": "Desi Hood",
        "src": "music/Desi%20Hood%20-%20Krish%20Rao%20(Music%20Video)%20%C2%A6%20Gully%20Gang%20Records%20%C2%A6%20Latest%20Haryanvi%20Song%202024%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "89"
    },
    {
        "name": "Kufar I ¦ Diljit Dosanjh I Manushi Chhillar ¦ Aura I Raj Ranjodh I MixSingh",
        "artist": "Unknown Artist",
        "src": "music/Kufar%20I%20Official%20Video%20%C2%A6%20Diljit%20Dosanjh%20I%20Manushi%20Chhillar%20%C2%A6%20Aura%20I%20Raj%20Ranjodh%20I%20MixSingh%20(152kbit_Opus).mp3",
        "color": "#ef4444",
        "id": "196"
    },
    {
        "name": "Kurchi Madathapetti Song ¦ Guntur Kaaram ¦ Mahesh Babu ¦ Sreeleela ¦ Trivikram ¦ Thaman S",
        "artist": "Unknown Artist",
        "src": "music/Kurchi%20Madathapetti%20Full%20Video%20Song%20%C2%A6%20Guntur%20Kaaram%20%C2%A6%20Mahesh%20Babu%20%C2%A6%20Sreeleela%20%C2%A6%20Trivikram%20%C2%A6%20Thaman%20S%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "197"
    },
    {
        "name": "Kya Baat Ay ¦ Jaani ¦ B Praak ¦ Arvindr Khaira ¦ Music",
        "artist": "Harrdy Sandhu",
        "src": "music/Harrdy%20Sandhu%20-%20Kya%20Baat%20Ay%20%C2%A6%20Jaani%20%C2%A6%20B%20Praak%20%C2%A6%20Arvindr%20Khaira%20%C2%A6%20Official%20Music%20Video%20(152kbit_Opus).mp3",
        "color": "#f97316",
        "id": "149"
    },
    {
        "name": "Lamba Lamba Kamba Meeda Song ¦ Folk Songs ¦ superhit dj song ¦Clement DJ Folk Song ¦ SVC",
        "artist": "Unknown Artist",
        "src": "music/Lamba%20Lamba%20Kamba%20Meeda%20Video%20Song%20%C2%A6%20Folk%20Video%20Songs%20%C2%A6%20superhit%20dj%20song%20%C2%A6Clement%20DJ%20Folk%20Song%20%C2%A6%20SVC%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "199"
    },
    {
        "name": "Let Me Love You feat Justin Bieber",
        "artist": "DJ Snake",
        "src": "music/DJ%20Snake%20-%20Let%20Me%20Love%20You%20ft.%20Justin%20Bieber%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "104"
    },
    {
        "name": "LET THE WORLD BURN",
        "artist": "Chris Grey",
        "src": "music/Chris%20Grey%20-%20LET%20THE%20WORLD%20BURN%20(Official%20Lyric%20Video)%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "73"
    },
    {
        "name": "Life in Rio",
        "artist": "Unknown Artist",
        "src": "music/Life%20in%20Rio%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "203"
    },
    {
        "name": "Lokiverse 2 0",
        "artist": "Unknown Artist",
        "src": "music/Lokiverse%202.0%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "207"
    },
    {
        "name": "lovely",
        "artist": "Billie Eilish, Khalid",
        "src": "music/Billie%20Eilish%2C%20Khalid%20-%20lovely%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "56"
    },
    {
        "name": "LOVELY BASTARDS",
        "artist": "ZWE1HVNDXR, yatashigang",
        "src": "music/ZWE1HVNDXR%2C%20yatashigang%20-%20LOVELY%20BASTARDS%20(152kbit_Opus).mp3",
        "color": "#06b6d4",
        "id": "353"
    },
    {
        "name": "Lucenzo And Don Omar Danza Kuduro",
        "artist": "Unknown Artist",
        "src": "music/Lucenzo%20And%20Don%20Omar%20Danza%20Kuduro%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "211"
    },
    {
        "name": "Lyrical ¦ Atif Aslam ¦ Ajab Prem Ki Ghazab Kahani ¦ 2009",
        "artist": "Tu Jaane Na",
        "src": "music/Tu%20Jaane%20Na%20-%20Lyrical%20%C2%A6%20Atif%20Aslam%20%C2%A6%20Ajab%20Prem%20Ki%20Ghazab%20Kahani%20%C2%A6%202009%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "334"
    },
    {
        "name": "Lyrical ¦ Court ¦ Priyadarshi, Harsh Roshan, Sridevi ¦ Vijai Bulganin ¦ Ram Jagadeesh",
        "artist": "Premalo",
        "src": "music/Premalo%20-%20Lyrical%20%C2%A6%20Court%20%C2%A6%20Priyadarshi%2C%20Harsh%20Roshan%2C%20Sridevi%20%C2%A6%20Vijai%20Bulganin%20%C2%A6%20Ram%20Jagadeesh%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "261"
    },
    {
        "name": "Lyrical ¦ Kanguva ¦ Suriya ¦ Devi Sri Prasad ¦ Siva ¦ Anurag Kulkarni",
        "artist": "Fire Song",
        "src": "music/Fire%20Song%20(Telugu)%20-%20Lyrical%20%C2%A6%20Kanguva%20%C2%A6%20Suriya%20%C2%A6%20Devi%20Sri%20Prasad%20%C2%A6%20Siva%20%C2%A6%20Anurag%20Kulkarni%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "135"
    },
    {
        "name": "Lyrical ¦ Laila Majnu ¦ Jyotica Tangri ¦ Avinash Tiwary & Tripti Dimri",
        "artist": "O Meri Laila",
        "src": "music/O%20Meri%20Laila%20-%20Lyrical%20%C2%A6%20Laila%20Majnu%20%C2%A6%20Jyotica%20Tangri%20%C2%A6%20Avinash%20Tiwary%20%26%20Tripti%20Dimri%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "243"
    },
    {
        "name": "Lyrical ¦ Raees ¦ Shah Rukh Khan ¦ Arijit Singh & Harshdeep K ¦ JAM8-Pritam",
        "artist": "Zaalima",
        "src": "music/Zaalima%20-%20Lyrical%20%C2%A6%20Raees%20%C2%A6%20Shah%20Rukh%20Khan%20%C2%A6%20Arijit%20Singh%20%26%20Harshdeep%20K%20%C2%A6%20JAM8-Pritam%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "352"
    },
    {
        "name": "Maate Vinadhuga Song ¦ Vijay Deverakonda, Priyanka Jawalkar ¦ Sid Sriram ¦ Jakes Bejoy",
        "artist": "Unknown Artist",
        "src": "music/Maate%20Vinadhuga%20Full%20Video%20Song%20%C2%A6%20Vijay%20Deverakonda%2C%20Priyanka%20Jawalkar%20%C2%A6%20Sid%20Sriram%20%C2%A6%20Jakes%20Bejoy%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "215"
    },
    {
        "name": "Mahiye Jinna Sohna Lyrical ¦ Darshan Raval ¦ Lijo George ¦ Dard ¦ Naushad Khan",
        "artist": "Unknown Artist",
        "src": "music/Mahiye%20Jinna%20Sohna%20Official%20Lyrical%20Video%20%C2%A6%20Darshan%20Raval%20%C2%A6%20Lijo%20George%20%C2%A6%20Dard%20%C2%A6%20Naushad%20Khan%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "216"
    },
    {
        "name": "Main Tera Boyfriend Song ¦ Raabta ¦ Arijit S ¦ Neha K Meet Bros ¦ Sushant Singh Rajput Kri…",
        "artist": "Unknown Artist",
        "src": "music/Main%20Tera%20Boyfriend%20Song%20%C2%A6%20Raabta%20%C2%A6%20Arijit%20S%20%C2%A6%20Neha%20K%20Meet%20Bros%20%C2%A6%20Sushant%20Singh%20Rajput%20Kriti%20Sanon%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "217"
    },
    {
        "name": "Manoharudu - Nuvvunte Naa Jathagaa ¦ Vikram, Amy Jackson ¦ A R Rahman",
        "artist": "I",
        "src": "music/I%20-%20Manoharudu%20-%20Nuvvunte%20Naa%20Jathagaa%20Video%20%C2%A6%20Vikram%2C%20Amy%20Jackson%20%C2%A6%20A.R.%20Rahman%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "161"
    },
    {
        "name": "Mast Magan Song ¦ 2 States ¦ Arijit Singh ¦ Arjun Kapoor, Alia Bhatt",
        "artist": "Unknown Artist",
        "src": "music/Mast%20Magan%20FULL%20Video%20Song%20%C2%A6%202%20States%20%C2%A6%20Arijit%20Singh%20%C2%A6%20Arjun%20Kapoor%2C%20Alia%20Bhatt%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "221"
    },
    {
        "name": "MATARGASHTI Song ¦ TAMASHA Songs 2015 ¦ Ranbir Kapoor, Deepika Padukone ¦ T-Series",
        "artist": "Unknown Artist",
        "src": "music/MATARGASHTI%20full%20VIDEO%20Song%20%C2%A6%20TAMASHA%20Songs%202015%20%C2%A6%20Ranbir%20Kapoor%2C%20Deepika%20Padukone%20%C2%A6%20T-Series%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "222"
    },
    {
        "name": "MELODIA ENVOLVENTE 4 2021",
        "artist": "MONTAGEM",
        "src": "music/MONTAGEM%20-%20MELODIA%20ENVOLVENTE%204%20%5B%20DJ%20DUDAH%20%5D%202021%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "230"
    },
    {
        "name": "Metro…In Dino; Zamaana Lage ¦ Anurag Basu ¦ Pritam ¦ Arijit Singh, Shashwat S ¦ Bhushan Ku…",
        "artist": "Unknown Artist",
        "src": "music/Metro%E2%80%A6In%20Dino%3B%20Zamaana%20Lage%20(Song)%20%C2%A6%20Anurag%20Basu%20%C2%A6%20Pritam%20%C2%A6%20Arijit%20Singh%2C%20Shashwat%20S%20%C2%A6%20Bhushan%20Kumar%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "223"
    },
    {
        "name": "Mexican Phonk Eki",
        "artist": "Unknown Artist",
        "src": "music/Mexican%20Phonk%20Eki%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "224"
    },
    {
        "name": "Mi Gente",
        "artist": "J Balvin, Willy William",
        "src": "music/J%20Balvin%2C%20Willy%20William%20-%20Mi%20Gente%20(Official%20Video)%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "176"
    },
    {
        "name": "Middle of the Night",
        "artist": "Elley Duhé",
        "src": "music/Elley%20Duh%C3%A9%20-%20Middle%20of%20the%20Night%20(lyrics)%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "119"
    },
    {
        "name": "MILLIONAIRE SONG ; @YoYoHoneySingh ¦ GLORY ¦ BHUSHAN KUMAR",
        "artist": "Unknown Artist",
        "src": "music/MILLIONAIRE%20SONG%20(Full%20Video)%3B%20%40YoYoHoneySingh%20%20%C2%A6%20GLORY%20%C2%A6%20BHUSHAN%20KUMAR%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "225"
    },
    {
        "name": "Montagem Mysterious Game",
        "artist": "LXNGVX",
        "src": "music/LXNGVX%20-%20Montagem%20Mysterious%20Game%20(Official%20Music)%20(160kbit_Opus).mp3",
        "color": "#f97316",
        "id": "213"
    },
    {
        "name": "MONTERO",
        "artist": "Lil Nas X",
        "src": "music/Lil%20Nas%20X%20-%20MONTERO%20(Call%20Me%20By%20Your%20Name)%20(Official%20Video)%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "205"
    },
    {
        "name": "Moral of the Story",
        "artist": "Ashe",
        "src": "music/Ashe%20-%20Moral%20of%20the%20Story%20(Official%20Audio)%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "39"
    },
    {
        "name": "Music ¦ MC SQUARE",
        "artist": "Laado",
        "src": "music/Laado%20-%20Official%20Music%20Video%20%C2%A6%20MC%20SQUARE%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "198"
    },
    {
        "name": "Music Film I Amir Ameer I Faheem Abdullah I Rauhan Malik I Samreen Kaur I Mir Tafazul",
        "artist": "ISHQ",
        "src": "music/ISHQ%20-%20Music%20Film%20I%20Amir%20Ameer%20I%20Faheem%20Abdullah%20I%20Rauhan%20Malik%20I%20Samreen%20Kaur%20I%20Mir%20Tafazul%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "175"
    },
    {
        "name": "My Love Is Gone ¦ Allu Arjun ¦ Devi Sri Prasad",
        "artist": "Aarya-2",
        "src": "music/Aarya-2%20-%20My%20Love%20Is%20Gone%20Video%20%C2%A6%20Allu%20Arjun%20%C2%A6%20Devi%20Sri%20Prasad%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "13"
    },
    {
        "name": "Naah Goriye -Bala ¦ Ayushmann Khurrana ¦ Harrdy Sandhu ¦ Swasti Mehul ¦B Praak ¦ Jaani ¦ S…",
        "artist": "Unknown Artist",
        "src": "music/Naah%20Goriye%20-Bala%20%C2%A6%20Ayushmann%20Khurrana%20%C2%A6%20Harrdy%20Sandhu%20%C2%A6%20Swasti%20Mehul%20%C2%A6B%20Praak%20%C2%A6%20Jaani%20%C2%A6%20Sonam%20Bajwa%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "233"
    },
    {
        "name": "Narsingi Narsing Rao Anna Singing Live At Secunderabad Bonalu¦ Chuttu Muttu Hyderabadu Son…",
        "artist": "Unknown Artist",
        "src": "music/Narsingi%20Narsing%20Rao%20Anna%20Singing%20Live%20At%20Secunderabad%20%20Bonalu%C2%A6%20Chuttu%20Muttu%20Hyderabadu%20Song%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "235"
    },
    {
        "name": "Nashe Si Chadh Gayi ¦ ¦ Befikre, Ranveer Singh, Vaani Kapoor, Arijit Singh, Vishal-Shekhar",
        "artist": "Unknown Artist",
        "src": "music/Nashe%20Si%20Chadh%20Gayi%20%C2%A6%20Full%20Song%20%C2%A6%20Befikre%2C%20Ranveer%20Singh%2C%20Vaani%20Kapoor%2C%20Arijit%20Singh%2C%20Vishal-Shekhar%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "236"
    },
    {
        "name": "Nature ¦ Kabira ¦ Music ¦ NJ Nindaniya ¦ Haryanvi",
        "artist": "Unknown Artist",
        "src": "music/Nature%20%C2%A6%20Kabira%20%C2%A6%20Official%20Music%20Video%20%C2%A6%20NJ%20Nindaniya%20%C2%A6%20Haryanvi%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "237"
    },
    {
        "name": "Neelanjana Ray and Divya Kumar Performs On Sun Saathiya ¦ The Voice India Kids ¦ Episode 3…",
        "artist": "Unknown Artist",
        "src": "music/Neelanjana%20Ray%20and%20Divya%20Kumar%20Performs%20On%20Sun%20Saathiya%20%C2%A6%20The%20Voice%20India%20Kids%20%C2%A6%20Episode%2034%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "238"
    },
    {
        "name": "Nemesis",
        "artist": "RYLLZ",
        "src": "music/RYLLZ%20-%20Nemesis%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "282"
    },
    {
        "name": "Nijame Ne Lyrical ¦ Ooru Peru Bhairavakona ¦ Sundeep Kishan ¦ Sid Sriram ¦ VI Anand ¦ Shek…",
        "artist": "Unknown Artist",
        "src": "music/Nijame%20Ne%20Lyrical%20%C2%A6%20Ooru%20Peru%20Bhairavakona%20%C2%A6%20Sundeep%20Kishan%20%C2%A6%20Sid%20Sriram%20%C2%A6%20VI%20Anand%20%C2%A6%20Shekar%20Chandra%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "240"
    },
    {
        "name": "No Lie feat Dua Lipa",
        "artist": "Sean Paul",
        "src": "music/Sean%20Paul%20-%20No%20Lie%20ft.%20Dua%20Lipa%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "287"
    },
    {
        "name": "Notion",
        "artist": "The Rare Occasions",
        "src": "music/The%20Rare%20Occasions%20-%20Notion%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "319"
    },
    {
        "name": "Often",
        "artist": "The Weeknd",
        "src": "music/The%20Weeknd%20-%20Often%20(NSFW)%20(Official%20Video)%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "321"
    },
    {
        "name": "OG Glimpse ¦ Pawan Kalyan ¦ Sujeeth ¦ Thaman S ¦ DVV Danayya",
        "artist": "Hungry Cheetah",
        "src": "music/Hungry%20Cheetah%20-%20OG%20Glimpse%20%C2%A6%20Pawan%20Kalyan%20%C2%A6%20Sujeeth%20%C2%A6%20Thaman%20S%20%C2%A6%20DVV%20Danayya%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "159"
    },
    {
        "name": "Okey Oka Lokam Lyrical ¦ Sashi Songs ¦ Aadi ¦ Sid Sriram ¦ Srinivas Naidu Nadikatla ¦ Arun…",
        "artist": "Unknown Artist",
        "src": "music/Okey%20Oka%20Lokam%20Lyrical%20%C2%A6%20Sashi%20Songs%20%C2%A6%20Aadi%20%C2%A6%20Sid%20Sriram%20%C2%A6%20Srinivas%20Naidu%20Nadikatla%20%C2%A6%20Arun%20Chiluveru%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "248"
    },
    {
        "name": "OLD MONEY ¦ AP DHILLON ¦ SALMAN KHAN ¦ SANJAY DUTT ¦ SHINDA KAHLON",
        "artist": "Unknown Artist",
        "src": "music/OLD%20MONEY%20%C2%A6%20AP%20DHILLON%20%C2%A6%20SALMAN%20KHAN%20%C2%A6%20SANJAY%20DUTT%20%C2%A6%20SHINDA%20KAHLON%20(Official%20Music%20Video)%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "249"
    },
    {
        "name": "Old Town Road feat Billy Ray Cyrus",
        "artist": "Lil Nas X",
        "src": "music/Lil%20Nas%20X%20-%20Old%20Town%20Road%20(Official%20Video)%20ft.%20Billy%20Ray%20Cyrus%20(152kbit_Opus).mp3",
        "color": "#f97316",
        "id": "206"
    },
    {
        "name": "On & On ¦ Electronic Pop ¦ NCS - Copyright Free Music",
        "artist": "Cartoon, Jéja",
        "src": "music/Cartoon%2C%20J%C3%A9ja%20-%20On%20%26%20On%20(feat.%20Daniel%20Levi)%20%C2%A6%20Electronic%20Pop%20%C2%A6%20NCS%20-%20Copyright%20Free%20Music%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "64"
    },
    {
        "name": "On The Floor",
        "artist": "Jennifer Lopez, Pitbull",
        "src": "music/Jennifer%20Lopez%2C%20Pitbull%20-%20On%20The%20Floor%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "179"
    },
    {
        "name": "On Top Karan Aujla ¦ Yeah Proof ¦ New Punjabi Songs 2022",
        "artist": "Unknown Artist",
        "src": "music/On%20Top%20(Full%20Video)%20Karan%20Aujla%20%C2%A6%20Yeah%20Proof%20%C2%A6%20New%20Punjabi%20Songs%202022%20(152kbit_Opus).mp3",
        "color": "#22c55e",
        "id": "250"
    },
    {
        "name": "One Of The Girls",
        "artist": "The Weeknd, JENNIE & Lily Rose Depp",
        "src": "music/The%20Weeknd%2C%20JENNIE%20%26%20Lily%20Rose%20Depp%20-%20One%20Of%20The%20Girls%20(Official%20Audio)%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "324"
    },
    {
        "name": "Orange Songs - Ram Charan Tej, Genelia-Aditya Music Telugu",
        "artist": "Chilipiga Choosthavala Song With",
        "src": "music/Chilipiga%20Choosthavala%20Song%20With%20Lyrics%20-%20Orange%20Songs%20-%20Ram%20Charan%20Tej%2C%20Genelia-Aditya%20Music%20Telugu%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "71"
    },
    {
        "name": "Ordinary Person ¦ Thalapathy Vijay ¦ Anirudh Ravichander",
        "artist": "Leo",
        "src": "music/Leo%20-%20Ordinary%20Person%20Video%20%C2%A6%20Thalapathy%20Vijay%20%C2%A6%20Anirudh%20Ravichander%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "202"
    },
    {
        "name": "Pain",
        "artist": "Unknown Artist",
        "src": "music/Pain%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "255"
    },
    {
        "name": "Pav Dharia ¦ 4K ¦ Dance Hit ¦ Punjabi Songs ¦ #pavdharia #najanaja",
        "artist": "Na Ja",
        "src": "music/Na%20Ja%20-%20Pav%20Dharia%20(Official%20Video)%20%C2%A6%204K%20Video%20%C2%A6%20Dance%20Hit%20%C2%A6%20Punjabi%20Songs%20%C2%A6%20%23pavdharia%20%20%23najanaja%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "232"
    },
    {
        "name": "Peaky Blinder ¦ i am not outsider i'm a peaky blinder",
        "artist": "Otnicka",
        "src": "music/Otnicka%20-%20Peaky%20Blinder%20(lyrics)%20%C2%A6%20i%20am%20not%20outsider%20i'm%20a%20peaky%20blinder%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "254"
    },
    {
        "name": "PEELINGS ¦ Pushpa 2 The Rule ¦ Allu Arjun ¦ Rashmika Mandanna ¦ Sukumar ¦ DSP",
        "artist": "Unknown Artist",
        "src": "music/PEELINGS%20(Telugu%20Lyrical%20Video)%20%C2%A6%20Pushpa%202%20The%20Rule%20%C2%A6%20Allu%20Arjun%20%C2%A6%20Rashmika%20Mandanna%20%C2%A6%20Sukumar%20%C2%A6%20DSP%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "257"
    },
    {
        "name": "Pillaa Raa Song ¦ RX 100 ¦ Kartikeya ¦ Payal Rajput ¦ Anurag Kulkarni ¦ Chaitan Bharadwaj",
        "artist": "Unknown Artist",
        "src": "music/Pillaa%20Raa%20Video%20Song%20%C2%A6%20RX%20100%20%C2%A6%20Kartikeya%20%C2%A6%20Payal%20Rajput%20%C2%A6%20Anurag%20Kulkarni%20%C2%A6%20Chaitan%20Bharadwaj%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "258"
    },
    {
        "name": "Podala Podala Gatlameeda Pothujoodammo",
        "artist": "Unknown Artist",
        "src": "music/Podala%20Podala%20Gatlameeda%20Pothujoodammo%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "259"
    },
    {
        "name": "Priyuralu Pilichindi Telugu Movie ¦ Yemi Cheyamanduve Song ¦ Ajith ¦ Aishwarya Rai ¦ Tabu",
        "artist": "Unknown Artist",
        "src": "music/Priyuralu%20Pilichindi%20Telugu%20Movie%20%C2%A6%20Yemi%20Cheyamanduve%20Video%20Song%20%C2%A6%20Ajith%20%C2%A6%20Aishwarya%20Rai%20%C2%A6%20Tabu%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "262"
    },
    {
        "name": "Psycho Saiyaan ¦ Saaho ¦ Prabhas, Shraddha Kapoor ¦ Tanishk Bagchi, Dhvani Bhanushali, Sac…",
        "artist": "Unknown Artist",
        "src": "music/Psycho%20Saiyaan%20%C2%A6%20Saaho%20%C2%A6%20Prabhas%2C%20Shraddha%20Kapoor%20%C2%A6%20Tanishk%20Bagchi%2C%20Dhvani%20Bhanushali%2C%20Sachet%20Tandon%20(152kbit_Opus).mp3",
        "color": "#ef4444",
        "id": "263"
    },
    {
        "name": "Pushpa 2 The Rule; Kissik ¦ Allu Arjun, Sreeleela ¦ DSP ¦ Lothika, Sublahshini, Raqueeb",
        "artist": "Unknown Artist",
        "src": "music/Pushpa%202%20The%20Rule%3B%20Kissik%20(Full%20Audio)%20%C2%A6%20Allu%20Arjun%2C%20Sreeleela%20%C2%A6%20DSP%20%C2%A6%20Lothika%2C%20Sublahshini%2C%20Raqueeb%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "264"
    },
    {
        "name": "Pushpa Pushpa -Pushpa 2 The Rule ¦ Allu Arjun ¦Sukumar ¦Rashmika ¦Mika,Nakash ¦Fahadh F¦DS…",
        "artist": "Unknown Artist",
        "src": "music/Pushpa%20Pushpa%20(Lyrical)-Pushpa%202%20The%20Rule%20%C2%A6%20Allu%20Arjun%20%C2%A6Sukumar%20%C2%A6Rashmika%20%C2%A6Mika%2CNakash%20%C2%A6Fahadh%20F%C2%A6DSP%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "265"
    },
    {
        "name": "Qaafirana ¦ Kedarnath ¦ Sushant Rajput ¦ Sara Ali Khan ¦ Arijit Singh & Nikhita ¦ Amit Tri…",
        "artist": "Unknown Artist",
        "src": "music/Qaafirana%20%C2%A6%20Kedarnath%20%C2%A6%20Sushant%20Rajput%20%C2%A6%20Sara%20Ali%20Khan%20%C2%A6%20Arijit%20Singh%20%26%20Nikhita%20%C2%A6%20Amit%20Trivedi%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "266"
    },
    {
        "name": "Raabta With ¦ Agent Vinod ¦ Saif Ali Khan, Kareena Kapoor,Pritam",
        "artist": "Unknown Artist",
        "src": "music/Raabta%20(Kehte%20Hain%20Khuda)%20Full%20Song%20With%20Lyrics%20%C2%A6%20Agent%20Vinod%20%C2%A6%20Saif%20Ali%20Khan%2C%20Kareena%20Kapoor%2CPritam%20(152kbit_Opus).mp3",
        "color": "#7c3aed",
        "id": "267"
    },
    {
        "name": "Raat Ke Shikari Masoom Sharma ¦ New Haryanvi Song 2025 ¦ Sonotek Music",
        "artist": "Unknown Artist",
        "src": "music/Raat%20Ke%20Shikari%20(Official%20Video)%20Masoom%20Sharma%20%C2%A6%20New%20Haryanvi%20Song%202025%20%C2%A6%20Sonotek%20Music%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "269"
    },
    {
        "name": "Ram Ram ¦ MC SQUARE ¦ Hustle 2 0",
        "artist": "Unknown Artist",
        "src": "music/Ram%20Ram%20%C2%A6%20MC%20SQUARE%20%C2%A6%20Hustle%202.0%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "271"
    },
    {
        "name": "Ram Siya Ram Sachet Tandon ¦ Poonam Thakkar ¦ Shabbir Ahmed ¦ Sidhika Sharma ¦ T-Series",
        "artist": "Unknown Artist",
        "src": "music/Ram%20Siya%20Ram%20(Full%20Song)%20Sachet%20Tandon%20%C2%A6%20Poonam%20Thakkar%20%C2%A6%20Shabbir%20Ahmed%20%C2%A6%20Sidhika%20Sharma%20%C2%A6%20T-Series%20(152kbit_Opus).mp3",
        "color": "#f97316",
        "id": "272"
    },
    {
        "name": "Ranbir, Anushka, Aishwarya ¦ Arijit Singh¦ Pritam",
        "artist": "Ae Dil Hai Mushkil Title Track",
        "src": "music/Ae%20Dil%20Hai%20Mushkil%20Title%20Track%20Full%20Video%20-%20Ranbir%2C%20Anushka%2C%20Aishwarya%20%C2%A6%20Arijit%20Singh%C2%A6%20Pritam%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "17"
    },
    {
        "name": "RAPTURE",
        "artist": "Unknown Artist",
        "src": "music/RAPTURE%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "275"
    },
    {
        "name": "Rauf & Faik это ли счастье¿",
        "artist": "Unknown Artist",
        "src": "music/Rauf%20%26%20Faik%20%20%20%D1%8D%D1%82%D0%BE%20%D0%BB%D0%B8%20%D1%81%D1%87%D0%B0%D1%81%D1%82%D1%8C%D0%B5%C2%BF%20(Official%20Audio)%20(152kbit_Opus).mp3",
        "color": "#60a5fa",
        "id": "276"
    },
    {
        "name": "redrum",
        "artist": "21 Savage",
        "src": "music/21%20Savage%20-%20redrum%20(Official%20Music%20Video)%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "7"
    },
    {
        "name": "Riar Saab, @AbhijaySharma ¦",
        "artist": "Obsessed",
        "src": "music/Obsessed%20-%20Riar%20Saab%2C%20%40AbhijaySharma%20%20%C2%A6%20Official%20Audio%20(152kbit_Opus).mp3",
        "color": "#22c55e",
        "id": "244"
    },
    {
        "name": "Rise Up",
        "artist": "TheFatRat",
        "src": "music/TheFatRat%20-%20Rise%20Up%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "326"
    },
    {
        "name": "Rockabye",
        "artist": "Clean Bandit",
        "src": "music/Clean%20Bandit%20-%20Rockabye%20(feat.%20Sean%20Paul%20%26%20Anne-Marie)%20%5BOfficial%20Video%5D%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "75"
    },
    {
        "name": "Russian Bandana ¦ Deepesh Goyal ¦ VYRL Haryanvi",
        "artist": "Dhanda Nyoliwala",
        "src": "music/Dhanda%20Nyoliwala%20-%20Russian%20Bandana%20(Music%20Video)%20%C2%A6%20Deepesh%20Goyal%20%C2%A6%20VYRL%20Haryanvi%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "93"
    },
    {
        "name": "Saad Lamjerred¦¦ Lamallem ¦¦ LORDZI Music",
        "artist": "Unknown Artist",
        "src": "music/Saad%20Lamjerred%C2%A6%C2%A6%20Lamallem%20(lyrics)%20%C2%A6%C2%A6%20LORDZI%20Music%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "283"
    },
    {
        "name": "Sachin Jaat ¦ Babli Jhuriya ¦ Masoom Sharma & Swara Verma",
        "artist": "Gundey Dore Pe",
        "src": "music/Gundey%20Dore%20Pe%20(Official%20Video)%20-%20Sachin%20Jaat%20%C2%A6%20Babli%20Jhuriya%20%C2%A6%20Masoom%20Sharma%20%26%20Swara%20Verma%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "143"
    },
    {
        "name": "Safari",
        "artist": "Unknown Artist",
        "src": "music/Safari%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "284"
    },
    {
        "name": "Sajni ; Arijit Singh, Ram Sampath ¦ Laapataa Ladies ¦ Aamir Khan Productions",
        "artist": "Unknown Artist",
        "src": "music/Sajni%20(Song)%3B%20Arijit%20Singh%2C%20Ram%20Sampath%20%C2%A6%20Laapataa%20Ladies%20%C2%A6%20Aamir%20Khan%20Productions%20(152kbit_Opus).mp3",
        "color": "#7c3aed",
        "id": "285"
    },
    {
        "name": "Sanson Ki Mala Pe",
        "artist": "INDIA METAL",
        "src": "music/INDIA%20METAL%20-%20Sanson%20Ki%20Mala%20Pe%20(ft.%20Shan%20Padman)%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "170"
    },
    {
        "name": "São Paulo",
        "artist": "The Weeknd",
        "src": "music/The%20Weeknd%20-%20S%C3%A3o%20Paulo%20(Official%20Music%20Video)%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "322"
    },
    {
        "name": "Satisfya",
        "artist": "Imran Khan",
        "src": "music/Imran%20Khan%20-%20Satisfya%20(Official%20Music%20Video)%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "169"
    },
    {
        "name": "Say My Name",
        "artist": "David Guetta, Bebe Rexha & J Balvin",
        "src": "music/David%20Guetta%2C%20Bebe%20Rexha%20%26%20J%20Balvin%20-%20Say%20My%20Name%20(Official%20Video)%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "87"
    },
    {
        "name": "SCOPIN",
        "artist": "KORDHELL",
        "src": "music/KORDHELL%20-%20SCOPIN%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "195"
    },
    {
        "name": "See Me Fall feat Kensei Abbot",
        "artist": "Ro Ransom",
        "src": "music/Ro%20Ransom%20-%20See%20Me%20Fall%20ft.%20Kensei%20Abbot%20(Y2K%20Remix)%20(Lyrics)%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "278"
    },
    {
        "name": "Seethakaalam Song ¦¦ S⁄o Satyamurthy Songs ¦¦ Allu Arjun, Samantha, Nithya Menon",
        "artist": "Unknown Artist",
        "src": "music/Seethakaalam%20Full%20Video%20Song%20%C2%A6%C2%A6%20S%E2%81%84o%20Satyamurthy%20Video%20Songs%20%C2%A6%C2%A6%20Allu%20Arjun%2C%20Samantha%2C%20Nithya%20Menon%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "288"
    },
    {
        "name": "Señorita",
        "artist": "Shawn Mendes, Camila Cabello",
        "src": "music/Shawn%20Mendes%2C%20Camila%20Cabello%20-%20Se%C3%B1orita%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "292"
    },
    {
        "name": "Set Fire to the Rain",
        "artist": "Unknown Artist",
        "src": "music/Set%20Fire%20to%20the%20Rain%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "289"
    },
    {
        "name": "Shahrukh Khan,Rani Mukherjee¦Shafqat Amanat Ali¦Shankar Mahadevan",
        "artist": "Mitwa , KANK",
        "src": "music/Mitwa%20Full%20Video%2C%20KANK%20-%20Shahrukh%20Khan%2CRani%20Mukherjee%C2%A6Shafqat%20Amanat%20Ali%C2%A6Shankar%20Mahadevan%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "226"
    },
    {
        "name": "Shaky Sanju Rathod feat Isha Malviya ¦ G-Spark ¦ New #Marathi Songs 2025",
        "artist": "Unknown Artist",
        "src": "music/Shaky%20(%20Official%20%23Video%20)%20Sanju%20Rathod%20Ft.%20Isha%20Malviya%20%C2%A6%20G-Spark%20%C2%A6%20New%20%23Marathi%20Songs%202025%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "291"
    },
    {
        "name": "SHAM DI LAALI ¦ Basant Kur ¦ Navvi ¦ MY NAME IS STREET",
        "artist": "Wazir Patar",
        "src": "music/Wazir%20Patar%20-%20SHAM%20DI%20LAALI%20%C2%A6%20Basant%20Kur%20%C2%A6%20Navvi%20%C2%A6%20MY%20NAME%20IS%20STREET%20(152kbit_Opus).mp3",
        "color": "#f97316",
        "id": "347"
    },
    {
        "name": "Shape of You",
        "artist": "Ed Sheeran",
        "src": "music/Ed%20Sheeran%20-%20Shape%20of%20You%20(Official%20Music%20Video)%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "115"
    },
    {
        "name": "Shape of You x Naina",
        "artist": "Ed Sheeran & Diljit Dosanjh",
        "src": "music/Ed%20Sheeran%20%26%20Diljit%20Dosanjh%20-%20Shape%20of%20You%20x%20Naina%20(Live%20in%20Birmingham%202024)%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "114"
    },
    {
        "name": "She Make It Clap",
        "artist": "Soulja Boy",
        "src": "music/Soulja%20Boy%20%E2%80%93%20She%20Make%20It%20Clap%20(Official%20Video)%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "304"
    },
    {
        "name": "Shershaah ¦ Jasleen Royal ¦ Sidharth–Kiara ¦ B Praak ¦ Romy ¦ Anvita Dutt",
        "artist": "Ranjha Reprise",
        "src": "music/Ranjha%20Reprise%20-%20Shershaah%20%C2%A6%20Jasleen%20Royal%20%C2%A6%20Sidharth%E2%80%93Kiara%20%C2%A6%20B%20Praak%20%C2%A6%20Romy%20%C2%A6%20Anvita%20Dutt%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "274"
    },
    {
        "name": "Shinunoga E-Wa",
        "artist": "Unknown Artist",
        "src": "music/Shinunoga%20E-Wa%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "293"
    },
    {
        "name": "SHOWTIME",
        "artist": "SHADXWRXGE",
        "src": "music/SHADXWRXGE%20-%20SHOWTIME%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "290"
    },
    {
        "name": "Shubh",
        "artist": "No Love",
        "src": "music/No%20Love%20(Official%20Audio)%20-%20Shubh%20(152kbit_Opus).mp3",
        "color": "#fb7185",
        "id": "241"
    },
    {
        "name": "Shubh",
        "artist": "We Rollin",
        "src": "music/We%20Rollin%20(Official%20Audio)%20-%20Shubh%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "348"
    },
    {
        "name": "Sidhu Moose Wala's Emotional Journey; Dollar ¦ Dakuaan Da Munda ¦ ¦ #sidhumoosewala",
        "artist": "Unknown Artist",
        "src": "music/Sidhu%20Moose%20Wala's%20Emotional%20Journey%3B%20Dollar%20%C2%A6%20Dakuaan%20Da%20Munda%20%C2%A6%20Official%20Video%20%C2%A6%20%23sidhumoosewala%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "300"
    },
    {
        "name": "Sir Osthara ¦¦ Businessman Movie ¦¦ Mahesh Babu, Kajal Aggarwal ¦¦ Thaman S",
        "artist": "Unknown Artist",
        "src": "music/Sir%20Osthara%20Full%20Song%20%C2%A6%C2%A6%20Businessman%20Movie%20%C2%A6%C2%A6%20Mahesh%20Babu%2C%20Kajal%20Aggarwal%20%C2%A6%C2%A6%20Thaman.S%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "301"
    },
    {
        "name": "Sitara ¦ Aashiq Khan x Mitika Kanwar ¦ Denny ¦ Shloke Lal ¦ Hindi Song",
        "artist": "Unknown Artist",
        "src": "music/Sitara%20(Official%20Video)%20%C2%A6%20Aashiq%20Khan%20x%20Mitika%20Kanwar%20%C2%A6%20Denny%20%C2%A6%20Shloke%20Lal%20%C2%A6%20Hindi%20Song%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "302"
    },
    {
        "name": "Sky High ¦ Progressive House ¦ NCS - Copyright Free Music",
        "artist": "Elektronomia",
        "src": "music/Elektronomia%20-%20Sky%20High%20%C2%A6%20Progressive%20House%20%C2%A6%20NCS%20-%20Copyright%20Free%20Music%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "118"
    },
    {
        "name": "Skyfall",
        "artist": "Adele",
        "src": "music/Adele%20-%20Skyfall%20(Official%20Lyric%20Video)%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "16"
    },
    {
        "name": "Softly Karan Aujla ¦ Ikky ¦ Making Memories ¦ Latest Punjabi Songs 2023",
        "artist": "Unknown Artist",
        "src": "music/Softly%20(Lyric%20Video)%20Karan%20Aujla%20%C2%A6%20Ikky%20%C2%A6%20Making%20Memories%20%C2%A6%20Latest%20Punjabi%20Songs%202023%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "303"
    },
    {
        "name": "Song ¦ HIT 2 ¦ Adivi Sesh ¦ Meenakshi ¦ MM Sreelekha ¦ Sid Sriram",
        "artist": "Urike Urike",
        "src": "music/Urike%20Urike%20-%20Video%20Song%20%C2%A6%20HIT%202%20%C2%A6%20Adivi%20Sesh%20%C2%A6%20Meenakshi%20%C2%A6%20MM%20Sreelekha%20%C2%A6%20Sid%20Sriram%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "338"
    },
    {
        "name": "Song ¦ Kalki 2898 AD ¦ Prabhas ¦ Disha Patani ¦ Santhosh Narayanan",
        "artist": "Ta Takkara",
        "src": "music/Ta%20Takkara%20(Telugu)%20-%20Video%C2%A0Song%20%C2%A6%20Kalki%202898%20AD%20%C2%A6%20Prabhas%20%C2%A6%20Disha%20Patani%20%C2%A6%20Santhosh%20Narayanan%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "312"
    },
    {
        "name": "Song feat Sid Sriram ¦ Gauthamante Radham ¦ Neeraj Madhav ¦Ankit Menon ¦Anand Menon ¦4K",
        "artist": "UYIRE",
        "src": "music/UYIRE%20-%20Video%20Song%20Ft.%20Sid%20Sriram%20%C2%A6%20Gauthamante%20Radham%20%C2%A6%20Neeraj%20Madhav%20%C2%A6Ankit%20Menon%20%C2%A6Anand%20Menon%C2%A0%C2%A64K%20(152kbit_Opus).mp3",
        "color": "#f97316",
        "id": "339"
    },
    {
        "name": "Sugar & Brownies",
        "artist": "DHARIA",
        "src": "music/DHARIA%20-%20Sugar%20%26%20Brownies%20(by%20Monoir)%20%5BOfficial%20Video%5D%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "94"
    },
    {
        "name": "SUKHA ¦ GURLEZ AKHTAR ¦ CHANI NATTAN ¦ PRODGK",
        "artist": "8 ASLE",
        "src": "music/8%20ASLE%20-%20SUKHA%20%C2%A6%20GURLEZ%20AKHTAR%20%C2%A6%20CHANI%20NATTAN%20%C2%A6%20PRODGK%20(152kbit_Opus).mp3",
        "color": "#06b6d4",
        "id": "9"
    },
    {
        "name": "Summertime Sadness",
        "artist": "Lana Del Rey",
        "src": "music/Lana%20Del%20Rey%20-%20Summertime%20Sadness%20(Official%20Music%20Video)%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "200"
    },
    {
        "name": "SUNRISE",
        "artist": "Unknown Artist",
        "src": "music/SUNRISE%20(152kbit_Opus).mp3",
        "color": "#22c55e",
        "id": "309"
    },
    {
        "name": "Superman",
        "artist": "Eminem",
        "src": "music/Eminem%20-%20Superman%20(dirty%20version)%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "120"
    },
    {
        "name": "Suttamla Soosi Lyrical ¦ Gangs of Godavari ¦ VishwakSen, Neha Shetty ¦ Yuvan Shankar Raja",
        "artist": "Unknown Artist",
        "src": "music/Suttamla%20Soosi%20Lyrical%20Video%20%C2%A6%20Gangs%20of%20Godavari%20%C2%A6%20VishwakSen%2C%20Neha%20Shetty%20%C2%A6%20Yuvan%20Shankar%20Raja%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "310"
    },
    {
        "name": "Swalla",
        "artist": "Jason Derulo",
        "src": "music/Jason%20Derulo%20-%20Swalla%20(feat.%20Nicki%20Minaj%20%26%20Ty%20Dolla%20%24ign)%20%5BOfficial%20Music%20Video%5D%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "178"
    },
    {
        "name": "SWING ZARA Song ¦ Jai Lava Kusa Songs ¦ Jr NTR, Tamannaah ¦ Devi Sri Prasad",
        "artist": "Unknown Artist",
        "src": "music/SWING%20ZARA%20Full%20Video%20Song%20%C2%A6%20Jai%20Lava%20Kusa%20Video%20Songs%20%C2%A6%20Jr%20NTR%2C%20Tamannaah%20%C2%A6%20Devi%20Sri%20Prasad%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "311"
    },
    {
        "name": "Taare Zameen Par",
        "artist": "Kholo Kholo Film",
        "src": "music/Kholo%20Kholo%20(Full%20Song)%20Film%20-%20Taare%20Zameen%20Par%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "192"
    },
    {
        "name": "Takeover EP ¦ #shorts",
        "artist": "Noormahal",
        "src": "music/Noormahal%20-%20Takeover%20EP%20%C2%A6%20%23shorts%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "242"
    },
    {
        "name": "Taki Taki feat Selena Gomez, Ozuna, Cardi B",
        "artist": "DJ Snake",
        "src": "music/DJ%20Snake%20-%20Taki%20Taki%20ft.%20Selena%20Gomez%2C%20Ozuna%2C%20Cardi%20B%20(Official%20Music%20Video)%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "105"
    },
    {
        "name": "Teaser ¦ Crew ¦ Badshah ¦ Raj Ranjodh",
        "artist": "Naina x Diljit Dosanjh",
        "src": "music/Naina%20x%20Diljit%20Dosanjh%20%20-%20Teaser%20%C2%A6%20Crew%20%C2%A6%20Badshah%20%C2%A6%20Raj%20Ranjodh%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "234"
    },
    {
        "name": "Telisiney Na Nuvvey",
        "artist": "Unknown Artist",
        "src": "music/Telisiney%20Na%20Nuvvey%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "313"
    },
    {
        "name": "Telugu",
        "artist": "Hukum",
        "src": "music/Hukum%20-%20Telugu%20(From%20'Jailer')%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "158"
    },
    {
        "name": "Tere Ishk Mein ¦ Dhanush, Kriti S ¦ AR Rahman, Arijit Singh, Irshad K ¦ Aanand LR ¦ Bhusha…",
        "artist": "Unknown Artist",
        "src": "music/Tere%20Ishk%20Mein%20(Song)%20%C2%A6%20Dhanush%2C%20Kriti%20S%20%C2%A6%20AR%20Rahman%2C%20Arijit%20Singh%2C%20Irshad%20K%20%C2%A6%20Aanand%20LR%20%C2%A6%20Bhushan%20K%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "315"
    },
    {
        "name": "Teri Meri Kahaani Reprise ¦ Chirantan Bhatt feat Palak Muchhal , Manoj Yadav ¦ Gaurav Jang",
        "artist": "Unknown Artist",
        "src": "music/Teri%20Meri%20Kahaani%20Reprise%20%C2%A6%20Chirantan%20Bhatt%20ft.%20Palak%20Muchhal%20%2C%20Manoj%20Yadav%20%C2%A6%20Gaurav%20Jang%20(152kbit_Opus).mp3",
        "color": "#06b6d4",
        "id": "317"
    },
    {
        "name": "Teriyaki Boyz",
        "artist": "Tokyo Drift",
        "src": "music/Tokyo%20Drift%20-%20Teriyaki%20Boyz%20%5B%20MUSIC%20VIDEO%20%5D%20HD%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "329"
    },
    {
        "name": "THE AUTOTREM 1 0",
        "artist": "DJ RICK 013",
        "src": "music/DJ%20RICK%20013%20-%20THE%20AUTOTREM%201.0%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "103"
    },
    {
        "name": "THE FATE OF OPHELIA",
        "artist": "#TAYLORSWIFT",
        "src": "music/%23TAYLORSWIFT%20-%20THE%20FATE%20OF%20OPHELIA%20(Music%20Video)%20(152kbit_Opus).mp3",
        "color": "#22c55e",
        "id": "2"
    },
    {
        "name": "The Hills",
        "artist": "The Weeknd",
        "src": "music/The%20Weeknd%20-%20The%20Hills%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "323"
    },
    {
        "name": "The Real Slim Shady",
        "artist": "Eminem",
        "src": "music/Eminem%20-%20The%20Real%20Slim%20Shady%20(Official%20Video%20-%20Clean%20Version)%20(152kbit_Opus).mp3",
        "color": "#f97316",
        "id": "121"
    },
    {
        "name": "Thelusa Manasa",
        "artist": "Unknown Artist",
        "src": "music/Thelusa%20Manasa%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "327"
    },
    {
        "name": "THOMAS",
        "artist": "Unknown Artist",
        "src": "music/THOMAS%20(160kbit_Opus).mp3",
        "color": "#60a5fa",
        "id": "328"
    },
    {
        "name": "Thunder",
        "artist": "Imagine Dragons",
        "src": "music/Imagine%20Dragons%20-%20Thunder%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "168"
    },
    {
        "name": "Title Track ¦ Ranveer Singh, Shashwat Sachdev, Hanumankind, Jasmine Sandlas,Aditya Dhar",
        "artist": "Dhurandhar",
        "src": "music/Dhurandhar%20-%20Title%20Track%20%C2%A6%20Ranveer%20Singh%2C%20Shashwat%20Sachdev%2C%20Hanumankind%2C%20Jasmine%20Sandlas%2CAditya%20Dhar%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "96"
    },
    {
        "name": "Tony Kakkar feat Young Desi ¦ RE-UPLOADED AFTER 170 MILLION VIEWS",
        "artist": "Coca Cola Tu",
        "src": "music/Coca%20Cola%20Tu%20-%20Tony%20Kakkar%20ft.%20Young%20Desi%20%C2%A6%20RE-UPLOADED%20AFTER%20170%20MILLION%20VIEWS%20(152kbit_Opus).mp3",
        "color": "#f97316",
        "id": "76"
    },
    {
        "name": "TREINAMENTO DE FORÇA",
        "artist": "TRXVELER",
        "src": "music/TRXVELER%20-%20TREINAMENTO%20DE%20FOR%C3%87A%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "332"
    },
    {
        "name": "TU HI A",
        "artist": "Unknown Artist",
        "src": "music/TU%20HI%20A%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "333"
    },
    {
        "name": "TU JOGA PRA TRAS TU ENCOSTA ENCOSTA",
        "artist": "DJ Dn o Astro, Mc Mascara, Rodrigo do Cn",
        "src": "music/DJ%20Dn%20o%20Astro%2C%20Mc%20Mascara%2C%20Rodrigo%20do%20Cn%20-%20TU%20JOGA%20PRA%20TRAS%20TU%20ENCOSTA%20ENCOSTA%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "102"
    },
    {
        "name": "Tujh Mein Rab Dikhta Hai - Rab Ne Bana Di Jodi",
        "artist": "Flute",
        "src": "music/%5Binstrumental%5D%20Flute%20Version%20-%20Tujh%20Mein%20Rab%20Dikhta%20Hai%20-%20Rab%20Ne%20Bana%20Di%20Jodi%20(Aykronix%20Release)%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "354"
    },
    {
        "name": "Tum Hi Ho Soulful ¦ Mithoon ¦ MTV Unplugged",
        "artist": "Unknown Artist",
        "src": "music/Tum%20Hi%20Ho%20Soulful%20%C2%A6%20Mithoon%20%C2%A6%20MTV%20Unplugged%20(152kbit_Opus).mp3",
        "color": "#ef4444",
        "id": "335"
    },
    {
        "name": "twenty one pilots; Heathens",
        "artist": "Unknown Artist",
        "src": "music/twenty%20one%20pilots%3B%20Heathens%20(from%20Suicide%20Squad%3B%20The%20Album)%20%5BOFFICIAL%20VIDEO%5D%20(128kbit_AAC).mp3",
        "color": "#fb7185",
        "id": "336"
    },
    {
        "name": "ultraphunk",
        "artist": "Unknown Artist",
        "src": "music/ultraphunk%20(152kbit_Opus).mp3",
        "color": "#7c3aed",
        "id": "337"
    },
    {
        "name": "Understand",
        "artist": "BoyWithUke",
        "src": "music/BoyWithUke%20-%20Understand%20(Full%20Song%20Live)%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "59"
    },
    {
        "name": "Unholy",
        "artist": "Sam Smith, Kim Petras",
        "src": "music/Sam%20Smith%2C%20Kim%20Petras%20-%20Unholy%20(Official%20Music%20Video)%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "286"
    },
    {
        "name": "Unstoppable",
        "artist": "Sia",
        "src": "music/Sia%20-%20Unstoppable%20(Lyrics)%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "299"
    },
    {
        "name": "Uppenantha ¦ Allu Arjun ¦ Devi Sri Prasad",
        "artist": "Aarya-2",
        "src": "music/Aarya-2%20-%20Uppenantha%20Video%20%C2%A6%20Allu%20Arjun%20%C2%A6%20Devi%20Sri%20Prasad%20(152kbit_Opus).mp3",
        "color": "#ef4444",
        "id": "14"
    },
    {
        "name": "Uyire Uyire ¦ Song ¦ Anup Rubens ¦ Adnan Sami, Chinmayi",
        "artist": "Oh Priya Priya",
        "src": "music/Oh%20Priya%20Priya%20-%20Uyire%20Uyire%20%C2%A6%20%20Video%20Song%20%C2%A6%20Anup%20Rubens%20%C2%A6%20Adnan%20Sami%2C%20Chinmayi%20(152kbit_Opus).mp3",
        "color": "#06b6d4",
        "id": "247"
    },
    {
        "name": "Vaalu Kanuladaanaa ¦ Premikula Roju ¦ Unni Menon ¦ A R Rahman",
        "artist": "Unknown Artist",
        "src": "music/Vaalu%20Kanuladaanaa%20(Telugu)%20%C2%A6%20Premikula%20Roju%20%C2%A6%20Unni%20Menon%20%C2%A6%20A.R.Rahman%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "340"
    },
    {
        "name": "Vaaste Song; Dhvani Bhanushali, Tanishk Bagchi ¦ Nikhil D'Souza ¦ Bhushan Kumar ¦ Radhika…",
        "artist": "Unknown Artist",
        "src": "music/Vaaste%20Song%3B%20Dhvani%20Bhanushali%2C%20Tanishk%20Bagchi%20%C2%A6%20Nikhil%20D'Souza%20%C2%A6%20Bhushan%20Kumar%20%C2%A6%20Radhika%20R%2C%20Vinay%20S%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "341"
    },
    {
        "name": "Varinder Brar ¦ New Punjabi Song 2021 ¦ Latest punjabi songs 2021",
        "artist": "12 Bande",
        "src": "music/12%20Bande%20-%20Varinder%20Brar%20(Official%20Video)%20%C2%A6%20New%20Punjabi%20Song%202021%20%C2%A6%20Latest%20punjabi%20songs%202021%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "6"
    },
    {
        "name": "Vellipomaake Song ¦ Saahasam Swaasaga Saagipo Songs ¦ NagaChaitanya, Manjima",
        "artist": "Unknown Artist",
        "src": "music/Vellipomaake%20Full%20Video%20Song%20%C2%A6%20Saahasam%20Swaasaga%20Saagipo%20Full%20Video%20Songs%20%C2%A6%20NagaChaitanya%2C%20Manjima%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "343"
    },
    {
        "name": "VICTORY ANTHEM- Khushi× @lashcurry prod by @AudiocrackerrBeatz1",
        "artist": "Unknown Artist",
        "src": "music/VICTORY%20ANTHEM-%20Khushi%C3%97%20%40lashcurry%20%20prod.by%20%40AudiocrackerrBeatz1%20%5BOfficial%20Audio%5D%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "345"
    },
    {
        "name": "Viral Attitude Song 👿👿",
        "artist": "Moti Chain Mota Paisa",
        "src": "music/Moti%20Chain%20Mota%20Paisa%20%5BSlowed%20Reverb%5D%20-%20Viral%20Attitude%20Song%20%F0%9F%91%BF%F0%9F%91%BF%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "231"
    },
    {
        "name": "Wahran",
        "artist": "RANDALL",
        "src": "music/RANDALL%20-%20Wahran%20(Official%20Audio)%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "273"
    },
    {
        "name": "Wakhra Swag ¦ ¦ Navv Inder feat Badshah ¦ Aman Hundal ¦ Best Punjabi Dance Song",
        "artist": "Unknown Artist",
        "src": "music/Wakhra%20Swag%20%C2%A6%20Official%20Video%20%C2%A6%20Navv%20Inder%20feat.%20Badshah%20%C2%A6%20Aman%20Hundal%20%C2%A6%20Best%20Punjabi%20Dance%20Song%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "346"
    },
    {
        "name": "We Don't Talk Anymore",
        "artist": "Charlie Puth",
        "src": "music/Charlie%20Puth%20-%20We%20Don't%20Talk%20Anymore%20(feat.%20Selena%20Gomez)%20%5BOfficial%20Video%5D%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "68"
    },
    {
        "name": "Winning Speech Karan Aujla ¦ Mxrci ¦ Latest Punjabi Songs 2024",
        "artist": "Unknown Artist",
        "src": "music/Winning%20Speech%20(Music%20Video)%20Karan%20Aujla%20%C2%A6%20Mxrci%20%C2%A6%20Latest%20Punjabi%20Songs%202024%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "350"
    },
    {
        "name": "Woops",
        "artist": "Bountyhunter",
        "src": "music/Bountyhunter%20-%20Woops%20(Original%20Mix)%20(%E2%99%A51993)%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "58"
    },
    {
        "name": "Worth It feat Kid Ink",
        "artist": "Fifth Harmony",
        "src": "music/Fifth%20Harmony%20-%20Worth%20It%20(Official%20Video)%20ft.%20Kid%20Ink%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "134"
    },
    {
        "name": "Yeh Tune Kya Kia by Javed Bashir",
        "artist": "Unknown Artist",
        "src": "music/Yeh%20Tune%20Kya%20Kia%20by%20Javed%20Bashir%20(128kbit_AAC).mp3",
        "color": "#7c3aed",
        "id": "351"
    },
    {
        "name": "Young and Beautiful",
        "artist": "Lana Del Rey",
        "src": "music/Lana%20Del%20Rey%20-%20Young%20and%20Beautiful%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "201"
    },
    {
        "name": "YUM YUM",
        "artist": "LXNGVX",
        "src": "music/LXNGVX%20-%20YUM%20YUM%20(Official%20Music)%20(152kbit_Opus).mp3",
        "color": "#f97316",
        "id": "214"
    },
    {
        "name": "Дикая львица",
        "artist": "ALEX & RUS",
        "src": "music/ALEX%20%26%20RUS%20%E2%80%93%20%D0%94%D0%B8%D0%BA%D0%B0%D1%8F%20%D0%BB%D1%8C%D0%B2%D0%B8%D1%86%D0%B0%20(Official%20Video%202019)%20(128kbit_AAC).mp3",
        "color": "#ef4444",
        "id": "25"
    },
    {
        "name": "Там Ревели Горы",
        "artist": "Miyagi & Andy Panda",
        "src": "music/Miyagi%20%26%20Andy%20Panda%20-%20%D0%A2%D0%B0%D0%BC%20%D0%A0%D0%B5%D0%B2%D0%B5%D0%BB%D0%B8%20%D0%93%D0%BE%D1%80%D1%8B%20(Mood%20Video)%20(128kbit_AAC).mp3",
        "color": "#22c55e",
        "id": "227"
    },
    {
        "name": "Тлеет",
        "artist": "BULA & SVNV",
        "src": "music/BULA%20%26%20SVNV%20-%20%D0%A2%D0%BB%D0%B5%D0%B5%D1%82%20(%D0%9E%D1%84%D0%B8%D1%86%D0%B8%D0%B0%D0%BB%D1%8C%D0%BD%D0%B0%D1%8F%20%D0%BF%D1%80%D0%B5%D0%BC%D1%8C%D0%B5%D1%80%D0%B0%20%D1%82%D1%80%D0%B5%D0%BA%D0%B0)%20(152kbit_Opus).mp3",
        "color": "#ef4444",
        "id": "61"
    },
    {
        "name": "Фонтанчик с дельфином",
        "artist": "Гио Пика",
        "src": "music/%D0%93%D0%B8%D0%BE%20%D0%9F%D0%B8%D0%BA%D0%B0%20-%20%D0%A4%D0%BE%D0%BD%D1%82%D0%B0%D0%BD%D1%87%D0%B8%D0%BA%20%D1%81%20%D0%B4%D0%B5%D0%BB%D1%8C%D1%84%D0%B8%D0%BD%D0%BE%D0%BC%20(Adam%20Maniac%20Remix)%20(128kbit_AAC).mp3",
        "color": "#06b6d4",
        "id": "356"
    },
    {
        "name": "Шома тигр @MELOMAN-MUSIC",
        "artist": "asanrap",
        "src": "music/asanrap%20-%20%20%D0%A8%D0%BE%D0%BC%D0%B0%20%D1%82%D0%B8%D0%B3%D1%80%20(Single%202021)%20%40MELOMAN-MUSIC%20(128kbit_AAC).mp3",
        "color": "#60a5fa",
        "id": "38"
    },
    {
        "name": "भूतिया मार्ग Part 1 ¦ BHOOTIYA MAARG Part 1 ¦ Horror Story ¦ Chudail Ki Kahani",
        "artist": "Unknown Artist",
        "src": "music/%E0%A4%AD%E0%A5%82%E0%A4%A4%E0%A4%BF%E0%A4%AF%E0%A4%BE%20%E0%A4%AE%E0%A4%BE%E0%A4%B0%E0%A5%8D%E0%A4%97%20Part%201%20%C2%A6%20BHOOTIYA%20MAARG%20Part%201%20%C2%A6%20Horror%20Story%20%C2%A6%20Chudail%20Ki%20Kahani%20(128kbit_AAC).mp3",
        "color": "#f97316",
        "id": "357"
    }
];

/* ===== Storage helpers ===== */
const STORAGE_PLAYLISTS_KEY = 'ministream_playlists_v1';
function isAndroidApp(){ return typeof window.Android !== 'undefined' && typeof window.Android.loadData === 'function'; }
function loadPlaylists(){ try { const raw = isAndroidApp() ? (window.Android.loadData(STORAGE_PLAYLISTS_KEY) || null) : localStorage.getItem(STORAGE_PLAYLISTS_KEY); return raw ? JSON.parse(raw) : {}; } catch(e){ console.error('loadPlaylists', e); return {}; } }
function savePlaylists(obj){ try { const raw=JSON.stringify(obj); if (isAndroidApp()) window.Android.saveData(STORAGE_PLAYLISTS_KEY, raw); else localStorage.setItem(STORAGE_PLAYLISTS_KEY, raw); } catch(e){ console.error('savePlaylists', e); } }

/* ===== App state ===== */
let playlists = loadPlaylists();
let filteredSongs = songsDB.slice();
let currentSongId = null;
let currentPlaylistName = null;
let shuffleEnabled = false;
let repeatMode = 'off'; // 'off' | 'one' | 'all'

/* ===== DOM refs ===== */
const songListEl = document.getElementById('songList');
const searchInput = document.getElementById('searchInput');
const audio = document.getElementById('audio');
const miniPlayer = document.getElementById('miniPlayer');
const miniTitle = document.getElementById('miniTitle');
const miniArtist = document.getElementById('miniArtist');
const miniPlayToggle = document.getElementById('miniPlayToggle');
const miniPlayIcon = document.getElementById('miniPlayIcon');
const progressBar = document.getElementById('progressBar');
const progressContainer = document.getElementById('progressContainer');
const modalBackdrop = document.getElementById('modalBackdrop');
const libraryView = document.getElementById('libraryView');
const libraryList = document.getElementById('libraryList');
const homeView = document.getElementById('homeView');
const profileModal = document.getElementById('profileModal');
const currentTimeDisplay = document.getElementById('currentTimeDisplay');
const durationDisplay = document.getElementById('durationDisplay');
const btnNext = document.getElementById('btnNext');
const btnPrev = document.getElementById('btnPrev');
const btnShuffle = document.getElementById('btnShuffle');
const btnRepeat = document.getElementById('btnRepeat');
const miniCover = document.getElementById('miniCover');
const btnExpand = document.getElementById('btnExpand');

/* Fullscreen player refs */
const fsModal = document.getElementById('fullScreenPlayer');
const fsClose = document.getElementById('fsClose');
const fsCover = document.getElementById('fsCover');
const fsLargeCover = document.getElementById('fsLargeCover');
const fsTitle = document.getElementById('fsTitle');
const fsArtist = document.getElementById('fsArtist');
const fsPlay = document.getElementById('fsPlay');
const fsPrev = document.getElementById('fsPrev');
const fsNext = document.getElementById('fsNext');
const fsShuffle = document.getElementById('fsShuffle');
const fsRepeat = document.getElementById('fsRepeat');
const fsCurrent = document.getElementById('fsCurrent');
const fsDuration = document.getElementById('fsDuration');
const fsProgress = document.getElementById('fsProgress');
const fsProgressWrap = document.getElementById('fsProgressWrap');
const fsTabs = document.querySelectorAll('.tab-btn');
const fsPanels = document.querySelectorAll('.fs-panel');
const lyricsContent = document.getElementById('lyricsContent');

/* ===== Helpers ===== */
function formatTime(sec){ if (!sec || isNaN(sec)) return '0:00'; const m=Math.floor(sec/60); const s=Math.floor(sec%60).toString().padStart(2,'0'); return `${m}:${s}`; }
function createCoverText(name){ if(!name) return ''; const words=name.split(' ').filter(Boolean); if(words.length===1) return words[0].slice(0,2).toUpperCase(); return (words[0][0]+(words[1]?words[1][0]:'' )).toUpperCase(); }

/* ===== Now playing & UI updates ===== */
function updateNowPlaying(song){
  if(!song) return;
  miniTitle.textContent = song.name;
  miniArtist.textContent = song.artist;
  miniCover.style.background = `linear-gradient(135deg, ${song.color||'#666'}, rgba(255,255,255,0.02))`;
  miniCover.textContent = createCoverText(song.name);
  miniPlayer.style.display = 'flex';
  miniPlayer.setAttribute('aria-hidden','false');
  updateUIPlayingState();
  setupMarqueeIfNeeded();
}

/* set play/pause icon and button states */
function updateUIPlayingState(){
  const playing = audio && !audio.paused && !audio.ended && audio.src;
  if(miniPlayIcon){
    if(playing) miniPlayIcon.innerHTML = '<rect x="6" y="5" width="4" height="14" fill="white"/><rect x="14" y="5" width="4" height="14" fill="white"/>';
    else miniPlayIcon.innerHTML = '<path d="M5 3v18l15-9L5 3z" fill="white"/>';
  }
  if(btnShuffle) btnShuffle.classList.toggle('active', shuffleEnabled);
  if(btnRepeat){
    btnRepeat.classList.remove('mode-off','mode-one','mode-all');
    btnRepeat.classList.add(repeatMode==='off'?'mode-off':(repeatMode==='one'?'mode-one':'mode-all'));
  }
  // fullscreen play button
  if(fsPlay) fsPlay.textContent = (audio && !audio.paused && !audio.ended) ? '⏸' : '▶';
  if(fsShuffle) fsShuffle.classList.toggle('active', shuffleEnabled);
  if(fsRepeat){
    fsRepeat.classList.remove('mode-off','mode-one','mode-all');
    fsRepeat.classList.add(repeatMode==='off'?'mode-off':(repeatMode==='one'?'mode-one':'mode-all'));
  }
}

/* ===== Playback control ===== */
function togglePlaySong(songId){
  const song = songsDB.find(s=>s.id===songId);
  if(!song) return;
  if(currentSongId===songId){
    if(audio.paused) audio.play().catch(e=>console.warn('play blocked', e));
    else audio.pause();
  } else {
    currentSongId = songId;
    audio.src = song.src;
    if(song.src && song.src.toLowerCase().endsWith('.m4a')) audio.type = 'audio/mp4'; else audio.type = '';
    audio.currentTime = 0;
    audio.load();
    audio.play().catch(e=>console.error('playback failed', e));
    updateNowPlaying(song);
  }
  updateUIPlayingState();
  // update fullscreen if open
  populateFullScreen(song);
}

function playNextSong(){
  if(shuffleEnabled){
    const pool = currentPlaylistName && playlists[currentPlaylistName] && playlists[currentPlaylistName].length ? playlists[currentPlaylistName].slice() : songsDB.map(s=>s.id);
    if(!pool.length) return;
    let nextId;
    if(pool.length===1) nextId = pool[0];
    else {
      do { nextId = pool[Math.floor(Math.random()*pool.length)]; } while(nextId===currentSongId && pool.length>1);
    }
    togglePlaySong(nextId); return;
  }
  if(currentPlaylistName){
    const list = playlists[currentPlaylistName] || []; const idx = list.indexOf(currentSongId);
    if(idx>=0 && idx<list.length-1) togglePlaySong(list[idx+1]);
    else { if(repeatMode==='all') togglePlaySong(list[0]); else { audio.pause(); currentSongId=null; updateUIPlayingState(); } } return;
  }
  const ids = songsDB.map(s=>s.id); const idx = ids.indexOf(currentSongId);
  if(idx>=0 && idx<ids.length-1) togglePlaySong(ids[idx+1]); else { if(repeatMode==='all') togglePlaySong(ids[0]); else { audio.pause(); currentSongId=null; updateUIPlayingState(); } }
}

function playPrevSong(){
  if(!currentSongId) return;
  if(audio.currentTime>3){ audio.currentTime=0; return; }
  if(currentPlaylistName){ const list = playlists[currentPlaylistName]||[]; const idx=list.indexOf(currentSongId); if(idx>0) togglePlaySong(list[idx-1]); else togglePlaySong(list[0]); return; }
  const ids = songsDB.map(s=>s.id); const idx = ids.indexOf(currentSongId); if(idx>0) togglePlaySong(ids[idx-1]); else togglePlaySong(ids[0]);
}

/* ===== Render songs list ===== */
function renderSongs(list){
  songListEl.innerHTML='';
  if(!Array.isArray(list) || !list.length){ songListEl.innerHTML = '<div class="empty">No songs found. Adjust your search filter.</div>'; return; }
  list.forEach(song=>{
    const s = document.createElement('div'); s.className='song'; s.dataset.id = song.id;
    const cover = document.createElement('div'); cover.className='cover'; cover.style.background = `linear-gradient(135deg, ${song.color||'#666'}, rgba(255,255,255,0.02))`; cover.textContent = createCoverText(song.name);
    const meta = document.createElement('div'); meta.className='meta';
    const name = document.createElement('div'); name.className='name'; name.textContent = song.name;
    const artist = document.createElement('div'); artist.className='artist'; artist.textContent = song.artist;
    meta.appendChild(name); meta.appendChild(artist);
    const controls = document.createElement('div'); controls.className='controls';
    const playBtn = document.createElement('button'); playBtn.className='btn-circle'; playBtn.title = 'Play'; playBtn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M5 3v18l15-9L5 3z" fill="white"/></svg>`;
    playBtn.addEventListener('click', (e)=>{ e.stopPropagation(); togglePlaySong(song.id); });
    const playIndicator = document.createElement('div'); playIndicator.className='play-indicator'; playIndicator.style.display = (song.id===currentSongId && !audio.paused)?'block':'none';
    controls.appendChild(playBtn); controls.appendChild(playIndicator);
    s.appendChild(cover); s.appendChild(meta); s.appendChild(controls);
    s.addEventListener('click', ()=> togglePlaySong(song.id));
    songListEl.appendChild(s);
  });
}

/* ===== Event Listeners (mini controls) ===== */
miniPlayToggle.addEventListener('click', ()=>{
  if(!audio.src) return;
  if(audio.paused) audio.play().catch(e=>console.warn('play blocked',e)); else audio.pause();
});
if(btnNext) btnNext.addEventListener('click', (e)=>{ e.stopPropagation(); playNextSong(); });
if(btnPrev) btnPrev.addEventListener('click', (e)=>{ e.stopPropagation(); playPrevSong(); });
if(btnShuffle) btnShuffle.addEventListener('click', ()=>{ shuffleEnabled=!shuffleEnabled; updateUIPlayingState(); });
if(btnRepeat) btnRepeat.addEventListener('click', ()=>{ if(repeatMode==='off') repeatMode='all'; else if(repeatMode==='all') repeatMode='one'; else repeatMode='off'; updateUIPlayingState(); });

/* Expand to fullscreen */
if(btnExpand) btnExpand.addEventListener('click', (e)=>{ e.stopPropagation(); openFullScreen(); });
miniPlayer.addEventListener('click', (e)=>{ // open FS only if clicked not on a control
  const isControl = e.target.closest('.btn-circle') || e.target.closest('.mini-controls') || e.target.id==='progressContainer' || e.target.closest('.menu-list');
  if(!isControl) openFullScreen();
});

/* Seeking (mini) */
if(progressContainer) progressContainer.addEventListener('click', (e)=>{
  if(!audio.duration) return;
  const rect = progressContainer.getBoundingClientRect();
  const clickX = e.clientX - rect.left; const percent = Math.max(0, Math.min(1, clickX / rect.width));
  audio.currentTime = audio.duration * percent;
});

/* timeupdate */
audio.addEventListener('timeupdate', ()=>{
  if(!audio.duration || isNaN(audio.duration)){ currentTimeDisplay.textContent='0:00'; progressBar.style.width='0%'; fsCurrent.textContent='0:00'; fsProgress.style.width='0%'; return; }
  const pct = (audio.currentTime / audio.duration) * 100; progressBar.style.width = pct + '%'; currentTimeDisplay.textContent = formatTime(audio.currentTime); durationDisplay.textContent = formatTime(audio.duration);
  fsCurrent.textContent = formatTime(audio.currentTime); fsDuration.textContent = formatTime(audio.duration); fsProgress.style.width = pct + '%';
});

/* ended */
audio.addEventListener('ended', ()=>{
  if(repeatMode === 'one'){ audio.currentTime = 0; audio.play().catch(()=>{}); return; }
  playNextSong();
});

/* loadedmetadata/play/pause UI */
audio.addEventListener('loadedmetadata', updateUIPlayingState);
audio.addEventListener('play', updateUIPlayingState);
audio.addEventListener('pause', updateUIPlayingState);

/* search */
searchInput.addEventListener('input', (e)=>{ const q = e.target.value.trim().toLowerCase(); filteredSongs = songsDB.filter(s => ((s.name||'')+' '+(s.artist||'')).toLowerCase().includes(q)); renderSongs(filteredSongs); });

/* ===== Fullscreen player behaviors ===== */

function populateFullScreen(song){
  if(!song) return;
  fsCover.style.background = `linear-gradient(135deg, ${song.color||'#666'}, rgba(255,255,255,0.02))`;
  fsCover.textContent = createCoverText(song.name);
  fsLargeCover.style.background = `linear-gradient(135deg, ${song.color||'#666'}, rgba(255,255,255,0.02))`;
  fsLargeCover.textContent = createCoverText(song.name);
  fsTitle.textContent = song.name;
  fsArtist.textContent = song.artist;
  // description (optional)
  const desc = document.getElementById('fsDescription');
  if(desc) desc.textContent = `Now playing — ${song.name} • ${song.artist}`;
  // lyrics
  if(song.lyrics && song.lyrics.trim().length>0) lyricsContent.textContent = song.lyrics;
  else lyricsContent.textContent = 'No lyrics available for this song.';
  // set blurred backdrop color using song color
  document.getElementById('fsBackdrop').style.background = `linear-gradient(180deg, ${hexToRgba(song.color||'#000',0.18)}, rgba(0,0,0,0.65))`;
}

/* utility hex -> rgba */
function hexToRgba(hex, alpha){
  try {
    const h = hex.replace('#','');
    const r = parseInt(h.substring(0,2),16);
    const g = parseInt(h.substring(2,4),16);
    const b = parseInt(h.substring(4,6),16);
    return `rgba(${r},${g},${b},${alpha})`;
  } catch(e){ return `rgba(0,0,0,${alpha})`; }
}

/* open/close fullscreen */
function openFullScreen(){
  if(!currentSongId) return; // only if a song is loaded
  const song = songsDB.find(s=>s.id===currentSongId);
  populateFullScreen(song);
  fsModal.style.display = 'flex';
  fsModal.setAttribute('aria-hidden','false');
  // switch to player tab
  switchFsTab('player');
}
fsClose.addEventListener('click', closeFullScreen);
function closeFullScreen(){ fsModal.style.display='none'; fsModal.setAttribute('aria-hidden','true'); }

/* FS controls wiring */
if(fsPlay) fsPlay.addEventListener('click', ()=>{ if(!audio.src) return; if(audio.paused) audio.play().catch(()=>{}); else audio.pause(); });
if(fsPrev) fsPrev.addEventListener('click', ()=> playPrevSong());
if(fsNext) fsNext.addEventListener('click', ()=> playNextSong());
if(fsShuffle) fsShuffle.addEventListener('click', ()=>{ shuffleEnabled=!shuffleEnabled; updateUIPlayingState(); });
if(fsRepeat) fsRepeat.addEventListener('click', ()=>{ if(repeatMode==='off') repeatMode='all'; else if(repeatMode==='all') repeatMode='one'; else repeatMode='off'; updateUIPlayingState(); });

/* FS seek bar */
if(fsProgressWrap) fsProgressWrap.addEventListener('click', (e)=>{
  if(!audio.duration) return;
  const rect = fsProgressWrap.getBoundingClientRect();
  const clickX = e.clientX - rect.left;
  const percent = Math.max(0, Math.min(1, clickX / rect.width));
  audio.currentTime = audio.duration * percent;
});

/* FS tabs */
fsTabs.forEach(btn=>{
  btn.addEventListener('click', ()=>{ fsTabs.forEach(b=>b.classList.remove('active')); btn.classList.add('active'); const tab = btn.dataset.tab; switchFsTab(tab); });
});
function switchFsTab(tab){
  fsPanels.forEach(p=>p.style.display='none');
  const target = document.querySelector(`.fs-panel-${tab}`) || document.querySelector(`[data-panel="${tab}"]`);
  if(target) target.style.display='block';
}

/* ===== Playlist & Library (kept lightweight) ===== */
let openMenu = null;
function toggleMenuForSong(menuWrap, song){
  closeOpenMenu();
  const tpl = document.getElementById('menuTemplate').content.cloneNode(true);
  const menuList = tpl.querySelector('.menu-list');
  menuList.querySelector('[data-action="create"]').addEventListener('click',(e)=>{ e.stopPropagation(); const name = prompt('Create playlist name:'); if(name&&name.trim()) createPlaylist(name.trim(), [song.id]); closeOpenMenu(); });
  menuList.querySelector('[data-action="add"]').addEventListener('click',(e)=>{ e.stopPropagation(); showAddToPlaylistDialog(song.id); closeOpenMenu(); });
  menuWrap.appendChild(menuList); openMenu = menuList; setTimeout(()=> document.addEventListener('click', outsideMenuHandler),20);
}
function closeOpenMenu(){ if(openMenu && openMenu.parentNode) openMenu.parentNode.removeChild(openMenu); openMenu=null; document.removeEventListener('click', outsideMenuHandler); }
function outsideMenuHandler(e){ if(!openMenu) return; if(!openMenu.contains(e.target)) closeOpenMenu(); }

function createPlaylist(name, initialSongs=[]){
  if(!name) return; if(playlists[name]){ alert('A playlist with that name already exists.'); return; }
  playlists[name] = Array.from(new Set(initialSongs)); savePlaylists(playlists); if(libraryView.style.display==='block') renderLibraryIndex();
}
function showAddToPlaylistDialog(songId){
  const names = Object.keys(playlists||{});
  if(names.length===0){ if(confirm('No playlists exist yet. Create a new one?')){ const name = prompt('New playlist name:'); if(name) createPlaylist(name,[songId]); } return; }
  const listText = names.map((n,i)=>`${i+1}. ${n} (${playlists[n].length})`).join('\n');
  const pick = prompt('Add to which playlist? Enter number:\n'+listText); if(!pick) return; const idx = parseInt(pick.trim(),10)-1; if(isNaN(idx)||idx<0||idx>=names.length){ alert('Invalid selection'); return; }
  const pname = names[idx]; if(!playlists[pname]) playlists[pname]=[]; if(playlists[pname].includes(songId)){ alert(`Song is already in playlist "${pname}".`); return; }
  playlists[pname].push(songId); savePlaylists(playlists); alert(`Added to "${pname}"`);
}

/* Library rendering (kept similar to previous implementation) */
function renderLibraryIndex(){
  currentPlaylistName=null; libraryList.innerHTML='';
  const names = Object.keys(playlists);
  const header = document.createElement('div'); header.style.display='flex'; header.style.justifyContent='space-between'; header.style.alignItems='center'; header.style.marginBottom='12px';
  const title = document.createElement('div'); title.style.fontWeight='800'; title.style.fontSize='18px'; title.textContent='Your Playlists';
  const createNewBtn = document.createElement('button'); createNewBtn.className='btn-circle'; createNewBtn.style.background='var(--accent)'; createNewBtn.style.color='#000'; createNewBtn.innerHTML='✚'; createNewBtn.addEventListener('click',(e)=>{ e.stopPropagation(); const name = prompt('New playlist name:'); if(name&&name.trim()) createPlaylist(name.trim(),[]); });
  header.appendChild(title); header.appendChild(createNewBtn); libraryList.appendChild(header);
  if(names.length===0){ const empty = document.createElement('div'); empty.className='empty'; empty.textContent='No playlists yet. Click the ✚ to create one.'; libraryList.appendChild(empty); return; }
  names.forEach(name=>{
    const card = document.createElement('div'); card.className='playlist-card'; card.addEventListener('click', ()=> openPlaylistDetail(name));
    const thumb = document.createElement('div'); thumb.className='thumb'; thumb.textContent = name.split(' ').map(w => w[0]).slice(0,2).join('').toUpperCase();
    const meta = document.createElement('div'); meta.style.flex='1'; const titleEl = document.createElement('div'); titleEl.style.fontWeight='700'; titleEl.textContent = name;
    const count = document.createElement('div'); count.style.color='var(--muted)'; count.style.fontSize='13px'; count.textContent = `${(playlists[name]||[]).length} songs`;
    meta.appendChild(titleEl); meta.appendChild(count);
    const delBtn = document.createElement('button'); delBtn.className='btn-circle'; delBtn.innerHTML='🗑'; delBtn.title='Delete playlist'; delBtn.addEventListener('click',(e)=>{ e.stopPropagation(); if(confirm(`Delete playlist "${name}"? This cannot be undone.`)){ delete playlists[name]; savePlaylists(playlists); renderLibraryIndex(); } });
    card.appendChild(thumb); card.appendChild(meta); card.appendChild(delBtn); libraryList.appendChild(card);
  });
}
function openPlaylistDetail(name){
  currentPlaylistName=name; libraryList.innerHTML='';
  const playlistSongs = (playlists[name]||[]).map(id => songsDB.find(s=>s.id===id)).filter(Boolean);
  const header = document.createElement('div'); header.style.display='flex'; header.style.gap='16px'; header.style.alignItems='center'; header.style.marginBottom='16px';
  const backBtn = document.createElement('button'); backBtn.className='btn-circle'; backBtn.innerHTML='←'; backBtn.title='Back to Playlists'; backBtn.addEventListener('click', ()=> renderLibraryIndex());
  const title = document.createElement('h2'); title.style.fontWeight='800'; title.textContent = name; title.style.margin='0';
  header.appendChild(backBtn); header.appendChild(title); libraryList.appendChild(header);
  if(playlistSongs.length===0){ libraryList.innerHTML += `<div class="empty">Playlist empty. Add songs from Home by clicking the options menu (···) on a track.</div>`; return; }
  playlistSongs.forEach(song=>{
    const row=document.createElement('div'); row.className='song'; row.style.marginTop='8px'; row.dataset.id=song.id;
    const cover=document.createElement('div'); cover.className='cover'; cover.style.background = `linear-gradient(135deg, ${song.color||'#666'}, rgba(255,255,255,0.02))`; cover.textContent = createCoverText(song.name);
    const meta=document.createElement('div'); meta.className='meta'; const nameEl=document.createElement('div'); nameEl.className='name'; nameEl.textContent=song.name; const artist=document.createElement('div'); artist.className='artist'; artist.textContent=song.artist; meta.appendChild(nameEl); meta.appendChild(artist);
    const controls=document.createElement('div'); controls.className='controls';
    const playBtn=document.createElement('button'); playBtn.className='btn-circle'; playBtn.innerHTML=`<svg width="18" height="18" viewBox="0 0 24 24"><path d="M5 3v18l15-9L5 3z" fill="white"/></svg>`; playBtn.title='Play Song'; playBtn.addEventListener('click',(e)=>{ e.stopPropagation(); togglePlaySong(song.id); });
    const removeBtn=document.createElement('button'); removeBtn.className='btn-circle'; removeBtn.title='Remove from playlist'; removeBtn.innerHTML='−'; removeBtn.addEventListener('click',(e)=>{ e.stopPropagation(); if(confirm(`Remove "${song.name}" from playlist "${name}"?`)){ playlists[name] = (playlists[name]||[]).filter(x=>x!==song.id); savePlaylists(playlists); openPlaylistDetail(name); } });
    const playIndicator=document.createElement('div'); playIndicator.className='play-indicator'; playIndicator.style.display = (song.id===currentSongId && !audio.paused) ? 'block':'none';
    controls.appendChild(playBtn); controls.appendChild(playIndicator); controls.appendChild(removeBtn);
    row.appendChild(cover); row.appendChild(meta); row.appendChild(controls); row.addEventListener('click', ()=> togglePlaySong(song.id)); libraryList.appendChild(row);
  });
  updateUIPlayingState();
}

/* ===== Navigation logic ===== */
document.getElementById('navHome').addEventListener('click', ()=> setActiveNav('home'));
document.getElementById('navLibrary').addEventListener('click', ()=> setActiveNav('library'));
document.getElementById('navProfile').addEventListener('click', ()=> setActiveNav('profile'));

function setActiveNav(name){
  document.querySelectorAll('.bottom-nav .nav-btn').forEach(btn=>btn.classList.remove('active'));
  homeView.style.display='none'; libraryView.style.display='none'; searchInput.style.display='none'; modalBackdrop.style.display='none'; profileModal.style.display='none';
  if(name==='home'){ document.getElementById('navHome').classList.add('active'); homeView.style.display='block'; searchInput.style.display='block'; window.scrollTo(0,0); }
  else if(name==='library'){ document.getElementById('navLibrary').classList.add('active'); libraryView.style.display='block'; searchInput.style.display='block'; renderLibraryIndex(); window.scrollTo(0,0); }
  else if(name==='profile'){ document.getElementById('navProfile').classList.add('active'); modalBackdrop.style.display='flex'; profileModal.style.display='block'; }
}

/* ===== init ===== */
function init(){
  audio.volume = 1; audio.preload='metadata';
  renderSongs(songsDB);
  miniPlayer.style.display='none';
  const closeModalBtn = document.getElementById('closeModal');
  if(closeModalBtn) closeModalBtn.addEventListener('click', ()=> { modalBackdrop.style.display='none'; profileModal.style.display='none'; modalBackdrop.setAttribute('aria-hidden','true'); setActiveNav('home'); });
  setActiveNav('home');
  updateUIPlayingState();

  // FS close on backdrop click
  fsModal.addEventListener('click', (e)=>{ if(e.target===fsModal || e.target.id==='fsBackdrop') closeFullScreen(); });
}
init();

/* ===== Marquee helper ===== */
function setupMarqueeIfNeeded(){
  const wrap = document.querySelector('.marquee-wrap');
  const marquee = document.querySelector('.marquee');
  if(!wrap || !marquee) return;
  marquee.style.animation = 'none';
  marquee.dataset.scroll = 'false';
  marquee.dataset.duplicated = '';
  marquee.textContent = miniTitle.textContent;
  requestAnimationFrame(()=>{
    const wrapW = wrap.clientWidth;
    const txtW = marquee.scrollWidth;
    if(txtW > wrapW + 8){
      // duplicate
      marquee.textContent = miniTitle.textContent + '   •   ' + miniTitle.textContent;
      marquee.dataset.duplicated = 'true';
      marquee.dataset.scroll = 'true';
      const speed = 70;
      const duration = Math.max(8, (txtW / speed) * 2);
      marquee.style.animation = `marquee ${duration}s linear infinite`;
    } else {
      marquee.dataset.duplicated = ''; marquee.dataset.scroll='false'; marquee.textContent = miniTitle.textContent;
    }
  });
}

/* ===== Utility: cover text ===== */
function createCoverText(name){
  if(!name) return '';
  const words = name.split(' ').filter(Boolean);
  if(words.length===1) return words[0].slice(0,2).toUpperCase();
  return (words[0][0] + (words[1] ? words[1][0] : '')).toUpperCase();
}
